...
describe("Despeckle edge-preserving median (JS path)", () => {
  it("removes a salt impulse but keeps the surrounding field", () => {
    ...
    despeckle.func(fixture.canvas, { ...despeckleDefaults, radius: 2, threshold: 15, palette: identityPalette, _webglAcceleration: false } as never);
  ...
  it("preserves a step edge instead of blurring it", () => {
    ...
    const out = fixture.output();
    let lo = 255, hi = 0;
    for (let i = 0; i < out.length; i += 4) { lo = Math.min(lo, out[i]); hi = Math.max(hi, out[i]); }
  ...
  it("preserves source alpha and tolerates malformed options", () => {
    const fixture = makeCanvas(3, 3, Array.from({ length: 9 }, () => gray(120, 128)));
    expect(() => despeckle.func(fixture.canvas, { radius: Number.NaN, threshold: "x", palette: identityPalette, _webglAcceleration: false } as never)).not.toThrow();
    const out = fixture.output();
...
describe("Sharpen Gaussian unsharp mask (JS path)", () => {
  it("overshoots on both sides of a step edge (raises local contrast)", () => {
    ...
    sharpen.func(fixture.canvas, { ...sharpenDefaults, strength: 1.5, radius: 2, threshold: 0, palette: identityPalette, _webglAcceleration: false } as never);
    const out = fixture.output();
  ...
  it("leaves a flat region unchanged", () => {
    const fixture = makeCanvas(6, 1, Array.from({ length: 6 }, () => gray(128)));
    sharpen.func(fixture.canvas, { ...sharpenDefaults, strength: 2, radius: 3, threshold: 0, palette: identityPalette, _webglAcceleration: false } as never);
    const out = fixture.output();
    for (let i = 0; i < out.length; i += 4) expect(out[i]).toBe(128);
  });

  it("preserves source alpha", () => {
    const fixture = makeCanvas(8, 1, [40, 40, 40, 40, 200, 200, 200, 200].map((v) => gray(v, 96)));
    sharpen.func(fixture.canvas, { ...sharpenDefaults, palette: identityPalette, _webglAcceleration: false } as never);
    const out = fixture.output();
