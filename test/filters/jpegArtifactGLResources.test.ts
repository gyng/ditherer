...
describe("JPEG Artifact float texture ownership", () => {
  it("cleans either half-created resource and retries from an empty cache", () => {
    ...
    vi.mocked(gl.createTexture).mockReturnValueOnce(null).mockReturnValueOnce(texture);
    vi.mocked(gl.createFramebuffer).mockReturnValueOnce(framebuffer).mockReturnValueOnce(null);
