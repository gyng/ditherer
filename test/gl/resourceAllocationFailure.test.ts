...
const makeTextureGl = () => {
  ...
  return {
    ...
    createFramebuffer: vi.fn(() => ({ id: `framebuffer-${serial++}` }) as unknown as WebGLFramebuffer),
    deleteTexture: vi.fn(),
...
describe.each(textureAllocators)("$label texture ownership", ({
  allocate,
  keys,
  throwsOnMissingPair,
  ...
  ])("cleans the surviving handle when $missing allocation fails", ({ missing, texture, framebuffer }) => {
    const gl = makeTextureGl();
    vi.mocked(gl.createTexture).mockReturnValueOnce(texture as unknown as WebGLTexture | null);
    vi.mocked(gl.createFramebuffer).mockReturnValueOnce(framebuffer as unknown as WebGLFramebuffer | null);
...
describe("shader and program ownership", () => {
  ...
  it.each([
    ["attachShader", (gl: WebGL2RenderingContext) => vi.mocked(gl.attachShader).mockImplementationOnce(() => { throw new Error("injected attach failure"); })],
    ["bindAttribLocation", (gl: WebGL2RenderingContext) => vi.mocked(gl.bindAttribLocation).mockImplementationOnce(() => { throw new Error("injected bind failure"); })],
    ["linkProgram", (gl: WebGL2RenderingContext) => vi.mocked(gl.linkProgram).mockImplementationOnce(() => { throw new Error("injected link failure"); })],
  ...
  it("deletes the program and both shaders when the second attachment throws", () => {
    ...
    vi.mocked(gl.attachShader)
      ...
      .mockImplementationOnce(() => { throw new Error("injected second attach failure"); });
...
describe("FFT composite program ownership", () => {
  it.each([0, 1, 2, 3])("deletes all %i earlier programs when that link position fails", (failureIndex) => {
    const gl = makeProgramGl();
    ...
    const linker = vi.fn((_gl: WebGL2RenderingContext) => {
      ...
      const program = { prog: { id: `fft-${created.length}` } as unknown as WebGLProgram, uniforms: {} };
  ...
  it("transfers all four programs only after the complete bundle succeeds", () => {
    const gl = makeProgramGl();
    const linker = vi.fn((_gl: WebGL2RenderingContext, _source: string, uniforms: readonly string[]) => ({
...
describe("fullscreen quad composite ownership", () => {
  ...
  it.each([
    ["bindVertexArray", (gl: WebGL2RenderingContext) => vi.mocked(gl.bindVertexArray).mockImplementationOnce(() => { throw new Error("injected quad bind VAO failure"); })],
    ["bindBuffer", (gl: WebGL2RenderingContext) => vi.mocked(gl.bindBuffer).mockImplementationOnce(() => { throw new Error("injected quad bind buffer failure"); })],
    ["bufferData", (gl: WebGL2RenderingContext) => vi.mocked(gl.bufferData).mockImplementationOnce(() => { throw new Error("injected quad upload failure"); })],
    ["enableVertexAttribArray", (gl: WebGL2RenderingContext) => vi.mocked(gl.enableVertexAttribArray).mockImplementationOnce(() => { throw new Error("injected quad enable failure"); })],
    ["vertexAttribPointer", (gl: WebGL2RenderingContext) => vi.mocked(gl.vertexAttribPointer).mockImplementationOnce(() => { throw new Error("injected quad pointer failure"); })],
    ["final bindVertexArray", (gl: WebGL2RenderingContext) => vi.mocked(gl.bindVertexArray)
      .mockImplementationOnce(() => undefined)
      .mockImplementationOnce(() => { throw new Error("injected quad unbind failure"); })],
