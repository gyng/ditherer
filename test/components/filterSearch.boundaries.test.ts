...
const entries = [
  {
    ...
    category: "Photo",
    description: "Organic silver texture",
    keywords: "analog noise",
  },
  {
    displayName: "Glitch Blocks",
    category: "Digital Errors",
    description: "Broken compression",
    keywords: "datamosh",
  },
  {
    displayName: "Painterly Smear",
    category: "Artistic",
    description: "Painted brush strokes",
    keywords: "canvas",
  },
  { displayName: "Boxes", category: "Geometry", description: "Squared tiles", keywords: "grid" },
  {
    ...
    category: "Color",
    description: "Poster colors",
    keywords: "palette",
  },
  {
    displayName: "Grain Film",
    category: "Photo",
    description: "Ordering candidate",
    keywords: "analog",
  },
];
...
describe("filter search decision boundaries", () => {
  ...
  it("indexes absent optional text as empty searchable fields", () => {
    expect(buildFilterSearchIndex([{ displayName: "Bare", category: "Other" }])[0]).toMatchObject({
      description: "",
      keywords: "",
      nameWords: ["bare"],
      categoryWords: ["other"],
  ...
  it("orders exact names, name prefixes, name substrings, and category matches", () => {
    ...
    expect(searchFilterIndex(index, "film").items.map((entry) => entry.displayName)).toEqual([
      "Film Grain",
      "Grain Film",
    ]);
    expect(searchFilterIndex(index, "grain").items.map((entry) => entry.displayName)).toEqual([
      "Grain Film",
      "Film Grain",
    ]);
