...
import { setGlobalAudioVizModulation, type AudioVizConnection, type AudioVizMetric, type AudioVizSnapshot } from "utils/audioVizBridge";
import { notifyScreensaverChainSwap, notifyScreensaverVideoSwap, resetScreensaverSwapMarkers } from "utils/randomCycleBridge";
...
const change = (element: HTMLInputElement | HTMLSelectElement, value: string, checked?: boolean) => {
  act(() => {
...
const Harness = ({ empty = false }: { empty?: boolean }) => {
  const [connections, setConnections] = useState<AudioVizConnection[]>(empty ? [] : initialConnections);
...
describe("AudioPatchPanel interaction contract", () => {
  it("edits normalization, BPM override, density, sections, and collapsed state", async () => {
    ...
    change(mode, "chaotic");
    click(Array.from(container.querySelectorAll("button")).find((button) => button.textContent === "Reroll") ?? null);

    const settings = container.querySelector<HTMLButtonElement>('button[aria-label="Toggle auto viz settings"]')!;
    click(settings);
    expect(density.isConnected).toBe(false);
    click(settings);

    const tempoSection = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("Tempo & rhythm"))!;
    expect(tempoSection.getAttribute("aria-expanded")).toBe("true");
    click(tempoSection);
    expect(tempoSection.getAttribute("aria-expanded")).toBe("false");
    click(tempoSection);

    const normalize = Array.from(container.querySelectorAll<HTMLInputElement>('input[type="checkbox"]'))
    ...
    const override = Array.from(container.querySelectorAll<HTMLInputElement>('input[type="checkbox"]'))
      .find((input) => input.parentElement?.textContent?.includes("Override"))!;
    change(override, "", true);
    click(Array.from(container.querySelectorAll("button")).find((button) => button.textContent === "Tap") ?? null);
    click(Array.from(container.querySelectorAll("button")).find((button) => button.textContent === "Reset") ?? null);

    const collapse = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("Cable patch panel"))!;
    click(collapse);
  ...
  it("creates, removes, prompts, and drags cables while containing invalid gestures", async () => {
    ...
    const amountJack = container.querySelector<HTMLButtonElement>('button[title="Patch to Amount"]')!;
    const amountTarget = amountJack.closest("div")!;

    act(() => amountTarget.dispatchEvent(new MouseEvent("mouseup", { bubbles: true })));
    act(() => levelJack.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 0, clientX: 10, clientY: 10 })));
    act(() => window.dispatchEvent(new MouseEvent("mousemove", { bubbles: true, clientX: 20, clientY: 20 })));
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      act(() => label.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 0, clientY: 20 })));
    ...
    act(() => draggable.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 0, clientY: 100 })));
    ...
    act(() => remainingCable.dispatchEvent(new MouseEvent("contextmenu", { bubbles: true, cancelable: true })));
  ...
  it("renders the no-target state and controlled auto-viz callbacks", async () => {
    ...
    expect(Array.from(container.querySelectorAll<HTMLButtonElement>("button"))
      .find((button) => button.textContent === "Reroll")?.disabled).toBe(true);
  ...
  it("adds a new cable, ignores secondary-button drags, and falls back to option names", async () => {
    ...
    const targetJack = container.querySelector<HTMLButtonElement>('button[title="Patch to fallbackTarget"]')!;
    const target = targetJack.closest("div")!;
    act(() => metric.dispatchEvent(new MouseEvent("mousedown", {
      bubbles: true,
      button: 0,
      clientX: 5,
      clientY: 5,
    ...
    if (cableLabel) {
      act(() => cableLabel.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 2 })));
      act(() => window.dispatchEvent(new MouseEvent("mousemove", { bubbles: true, clientY: -200 })));
    }
