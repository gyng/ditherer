...
// const rust = import("@src/wasm/rgba2laba/wasm/rgba2laba");
...
export const serializeState = (state: RootState) => JSON.stringify(state);
...
export const referenceTable: {
  [k in ReferenceStandard]: { [k: string]: ReferenceValue };
} = {
  CIE_1931: {
    // 2° (CIE 1931)
...
export const colorDistance = (
  ...
  switch (colorDistanceAlgorithm) {
    case RGB_NEAREST as ColorDistanceAlgorithm:
      return Math.sqrt(
        (a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2 + (a[2] - b[2]) ** 2
      );
    case LAB_NEAREST as ColorDistanceAlgorithm: {
    ...
    case WASM_LAB_NEAREST as ColorDistanceAlgorithm: {
      return wasmRgbaLabaDistance(a, b);
    }
    case WASM_LAB_NEAREST_MEMO_PALETTE as ColorDistanceAlgorithm: {
    ...
    case HSV_NEAREST as ColorDistanceAlgorithm: {
    ...
    if (remaining <= 0) {
      switch (adaptMode) {
        case "AVERAGE": {
          ...
          return [acc.map((ch) => Math.floor(ch))] as Array<ColorRGBA>;
        }
        case "FIRST":
          return [bucket[0]];
        default:
...
export const uniqueColors = (
  ...
  const seen: { [k: string]: { count: number; color: ColorRGBA } } = {};
...
export const scaleMatrix = (
  mat: Array<Array<number | null | undefined>>,
  scale: number
...
export const brightness = (color: ColorRGBA, factor: number, exposure = 1) => [
...
export const cloneCanvas = (original: HTMLCanvasElement, copyData = true) => {
