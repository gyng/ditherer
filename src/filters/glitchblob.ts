...
import { cloneCanvas } from "@src/util";
import { filterImage } from "@src/actions";
import { ASYNC_FILTER } from "@src/constants/actionTypes";

import { BOOL, ENUM, RANGE } from "@src/constants/controlTypes";
...
const formatMap: { [k in Format]: string } = {
...
const computeCrc = (data: Uint8Array, crcBuf: Uint8Array) => {
  function buildCRC32Table(poly: any) {
...
const glitchblob = (
  ...
  const corruptThis = (
    ...
    return retry(10, () =>
      canvasToBlob(image, format as Format)
        .then(blobToUint8Array)
        .then(corruptor)
        .then((u8a) => new Blob([u8a], { type: formatMap[format as Format] }))
        .then(blobToImage)
    );
  };

  corruptThis(input, format as Format).then((image) => {
