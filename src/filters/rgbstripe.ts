...
import { BOOL, ENUM, RANGE, PALETTE } from "@src/constants/controlTypes";
...
import type { Palette, ColorRGBA } from "@src/types";
...
const masks: { [k in Mask]: (e: number) => Array<Array<Array<number>>> } = {
  // R G B
  [VERTICAL]: (e) => [
    [
      [1, e, e, 1],
...
const rgbStripe = (
  ...
  const mask = masks[shadowMask as Mask](effect);
  ...
  for (let x = 0; x < input.width; x += 1) {
    for (let y = 0; y < input.height; y += 1) {
      ...
      const brightnessAdjusted = brightnessFunc(
        masked,
        brightness,
        exposure
      ) as ColorRGBA;
      const contrastAdjusted = contrastFunc(
        brightnessAdjusted,
        contrast
      ) as ColorRGBA;
      const gammaAdjusted = gammaFunc(contrastAdjusted, gamma) as ColorRGBA;
