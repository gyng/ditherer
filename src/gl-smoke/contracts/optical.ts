...
export const runSharpenEdgeContrast = (): Result => {
  ...
  const r0 = range(before),
    r1 = range(after);
  return r1 > r0 + 5
    ? { ok: true }
    : {
        ok: false,
        reason: `Sharpen did not overshoot (range ${r0.toFixed(0)} -> ${r1.toFixed(0)})`,
      };
};
...
export const runFrequencyGaussianLowpass = (): Result => {
  ...
  const colLuma = (x: number): number => {
    ...
    for (let y = 0; y < h; y += 1) {
      const i = (y * w + x) * 4;
      s += 0.2126 * out[i] + 0.7152 * out[i + 1] + 0.0722 * out[i + 2];
    }
    return s / h;
  };
  ...
  if (!(c0 > c4 + 5 && c4 > c8 + 1)) {
    return {
      ok: false,
      reason: `low-pass not a centre-peaked Gaussian (profile ${c0.toFixed(1)}, ${c4.toFixed(1)}, ${c8.toFixed(1)} — a box would be flat)`,
    };
  }
  return { ok: true };
};
...
export const runBloomLinearGlow = (): Result => {
  ...
  if (!(meanLuma(glow) > meanLuma(before) + 0.5)) {
    return {
      ok: false,
      reason: `Bloom added no glow (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(glow).toFixed(2)})`,
    };
  }
  ...
  if (!(luma(glow, nearIdx) > 4)) {
    return {
      ok: false,
      reason: `Bloom glow did not spread to neighbours (${luma(glow, nearIdx).toFixed(1)})`,
    };
  }
  return Math.abs(meanLuma(inert) - meanLuma(before)) < 1
    ...
    : {
        ok: false,
        reason: `Bloom strength 0 not inert (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(inert).toFixed(2)})`,
      };
};
...
export const runMotionBlurLinearStreak = (): Result => {
  ...
  return boundaryLuma > naiveGammaMean + 20
    ...
    : {
        ok: false,
        reason: `Motion Blur boundary luma ${boundaryLuma.toFixed(1)} not brighter than gamma mean ${naiveGammaMean.toFixed(1)} + margin`,
      };
};
...
export const runRadialBlurLinearAverage = (): Result => {
  ...
  return mean > 155
    ...
    : {
        ok: false,
        reason: `Radial Blur averaged in gamma space, not linear (mean luma ${mean.toFixed(1)}, expected > 155)`,
      };
};
...
export const runLongExposureLinearAccumulation = (): Result => {
  ...
  const out = canvasPixels(
    filter.func(gray(bright), { ...base, _frameIndex: 1 }) as HTMLCanvasElement,
  );
  ...
  return measured > gammaMid + 20
    ...
    : {
        ok: false,
        reason: `shutter average ${measured.toFixed(0)} not linear-bright (gamma midpoint ${gammaMid}, expected > ${gammaMid + 20})`,
      };
};
...
export const runHalationLinearScreen = (): Result => {
  ...
  if (!(after[nearIdx] > after[nearIdx + 2] && after[nearIdx] > 10)) {
    return {
      ok: false,
      reason: `Halation neighbour not lifted/tinted (rgb ${after[nearIdx]},${after[nearIdx + 1]},${after[nearIdx + 2]})`,
    };
  }
  return meanLuma(after) > meanLuma(before) + 0.5
    ...
    : {
        ok: false,
        reason: `Halation added no glow (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(after).toFixed(2)})`,
      };
};
...
export const runOrtonLinearGlow = (): Result => {
  ...
  if (!(luma(glowed, nearIdx) > 8)) {
    return {
      ok: false,
      reason: `Orton glow did not lift the dark neighbour (${luma(glowed, nearIdx).toFixed(1)})`,
    };
  }
  return meanLuma(glowed) > meanLuma(before) + 0.5
    ...
    : {
        ok: false,
        reason: `Orton added no net glow (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(glowed).toFixed(2)})`,
      };
};
...
export const runTiltShiftLinearDefocus = (): Result => {
  ...
  const after = run("Tilt Shift", scene, {
    focusPosition: 0,
    focusWidth: 0.01,
    blurAmount: 8,
    saturationBoost: 0,
  ...
  return observed > gammaExpected + 8
    ...
    : {
        ok: false,
        reason: `Tilt Shift defocus not brighter than gamma baseline (observed ${observed.toFixed(1)}, gamma ${gammaExpected.toFixed(1)})`,
      };
};
...
export const runVolumetricLightLinearShafts = (): Result => {
  ...
  if (!before || !shafts || !inert)
    return { ok: false, reason: "Volumetric Light readback failed" };
  if (!(meanLuma(shafts) > meanLuma(before) + 0.5)) {
    return {
      ok: false,
      reason: `no shafts added (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(shafts).toFixed(2)})`,
    };
  }
  ...
  if (!(luma(shafts, midIdx) > 4)) {
    return {
      ok: false,
      reason: `shaft did not reach the dark field (${luma(shafts, midIdx).toFixed(1)})`,
    };
  }
  return Math.abs(meanLuma(inert) - meanLuma(before)) < 1
    ? { ok: true }
    : {
        ok: false,
        reason: `exposure 0 not inert (mean ${meanLuma(before).toFixed(2)} -> ${meanLuma(inert).toFixed(2)})`,
      };
};
...
export const runCcdChargeLinearBloom = (): Result => {
  ...
  const excessLin =
    Math.max(0, s2l(brightGamma) - thresholdLin) / Math.max(0.001, 1 - thresholdLin);
  ...
  for (let r = 1; r <= length; r += 1) {
    ...
    if (Math.abs(got - expectedGamma) > 8) {
      return {
        ok: false,
        reason: `row ${r}: expected ~${expectedGamma.toFixed(1)} (linear-weighted), got ${got}`,
      };
    }
  }
  return { ok: true };
...
export const runScanlineWarpAlphaWarped = (): Result => {
  const w = 32,
    h = 32;
  const src = paintCanvas(w, h, (x) => (x < w / 2 ? [220, 40, 40, 80] : [40, 220, 40, 255]));
...
export const runColorGradientNoiseAlphaPreserved = (): Result => {
  ...
  for (let i = 3; i < after.length; i += 4) {
    if (after[i] !== before[i]) {
      return {
        ok: false,
        reason: `Color Gradient Noise changed alpha at byte ${i} (${before[i]} -> ${after[i]})`,
      };
    }
  }
  return { ok: true };
...
export const runAnaglyphDuboisNoLeftLeak = (): Result => {
  const w = 64,
    h = 32;
  const stripe = paintCanvas(w, h, (x) => (x >= 20 && x < 28 ? [255, 0, 0, 255] : [0, 0, 0, 255]));
...
export const runConvolveEdgeClamp = (): Result => {
  ...
  const cpu = run("Convolve", field, {
    kernel: "SHARPEN_3X3",
    strength: 1,
    _webglAcceleration: false,
  });
  const gl = run("Convolve", field, {
    kernel: "SHARPEN_3X3",
    strength: 1,
  ...
  for (let y = 1; y < h - 1; y++) {
    const i = (w - 1 + w * y) * 4;
    maxDiff = Math.max(maxDiff, Math.abs(cpu[i] - gl[i]));
  }
  for (let x = 1; x < w - 1; x++) {
    const i = (x + w * (h - 1)) * 4;
    maxDiff = Math.max(maxDiff, Math.abs(cpu[i] - gl[i]));
  ...
  return maxDiff > 2
    ? { ok: false, reason: `CPU/GL edge clamp mismatch: max diff ${maxDiff} LSB` }
    : { ok: true };
};
...
export const runHalftoneCellAverageParity = (): Result => {
  ...
  const gL = meanLuma(grey),
    eL = meanLuma(edge),
    wL = meanLuma(white);
  if (eL >= wL * 0.9) {
    return {
      ok: false,
      reason: `edge cell dot not mid-tone: edge=${eL.toFixed(1)} vs white=${wL.toFixed(1)} (centre point-sample?)`,
    };
  }
  ...
  if (rel > 0.12) {
    return {
      ok: false,
      reason: `edge cell (${eL.toFixed(1)}) != grey cell (${gL.toFixed(1)}); GL not averaging the block`,
    };
  }
  return { ok: true };
};
...
export const runHalftoneLargeCellParity = (): Result => {
  ...
  const bL = meanLuma(biased),
    gL = meanLuma(grey),
    kL = meanLuma(black);
  if (Math.abs(bL - gL) > Math.max(2, gL * 0.2)) {
    return {
      ok: false,
      reason: `large cell dot != full-cell mean: biased=${bL.toFixed(1)} grey0.2=${gL.toFixed(1)}`,
    };
  }
  if (Math.abs(bL - kL) < Math.abs(bL - gL)) {
    return {
      ok: false,
      reason: `large cell dot tracks a top-left window (black=${kL.toFixed(1)}) not the full cell`,
    };
  }
  return { ok: true };
};
...
export const runHalftoneLinearizeParity = (): Result => {
  ...
  const lin = glRun(true),
    gam = glRun(false);
  if (!lin || !gam) return { ok: false, reason: "Halftone GL readback failed" };
  const lL = meanLuma(lin),
    gL = meanLuma(gam);
  if (lL <= gL + 2) {
    return {
      ok: false,
      reason: `GL ignores _linearize: linear=${lL.toFixed(1)} not brighter than gamma=${gL.toFixed(1)}`,
    };
  }
  return { ok: true };
};
...
export const runHalftoneAlphaFade = (): Result => {
  ...
  const glRun = (a: number) => {
    const out = filter.func(
      paintCanvas(S, S, () => [255, 255, 255, a]),
      {
        ...(filter.defaults ?? {}),
        ...runtimeOptions(),
        size: S,
  ...
  const faint = glRun(96),
    solid = glRun(255);
  if (!faint || !solid) return { ok: false, reason: "Halftone GL readback failed" };
  const fL = meanLuma(faint),
    sL = meanLuma(solid);
  if (fL >= sL - 2) {
    return {
      ok: false,
      reason: `GL ignores source alpha: alpha96=${fL.toFixed(1)} not fainter than opaque=${sL.toFixed(1)}`,
    };
  }
...
export const runXrayBeerLambert = (): Result => {
  ...
  for (const g of greys) {
    ...
    if (Math.abs(got - expected) > 6) {
      return {
        ok: false,
        reason: `grey ${g}: transmission ${got.toFixed(1)} != exp(-k*d) ${expected.toFixed(1)}`,
      };
    }
    measured.push(got);
  }
  for (let i = 1; i < measured.length; i += 1) {
    if (!(measured[i] < measured[i - 1] - 2)) {
      return {
        ok: false,
        reason: `not monotonic: grey ${greys[i - 1]} -> ${greys[i]} gave ${measured[i - 1].toFixed(1)} -> ${measured[i].toFixed(1)}`,
      };
    }
  }
  ...
  return dropA > dropB && dropB > dropC
    ...
    : {
        ok: false,
        reason: `falloff not exponential in density: drops ${dropA.toFixed(3)}, ${dropB.toFixed(3)}, ${dropC.toFixed(3)} should strictly shrink`,
      };
};
...
export const runSemEdgeBrightening = (): Result => {
  const w = 64,
    ...
    bright = 190,
    edge = w / 2;
  const step = paintCanvas(w, h, (x) => {
    const v = x < edge ? dark : bright;
    return [v, v, v, 255];
  });
  ...
  const flatDark = colLuma(8),
    flatBright = colLuma(56);
  let rim = 0,
    rimX = -1;
  ...
  if (!(rim > flatBright + 45 && rim > flatDark + 45)) {
    return {
      ok: false,
      reason: `no secant-law rim at stock defaults: edge ${rim.toFixed(1)} vs flats ${flatDark.toFixed(1)}/${flatBright.toFixed(1)}`,
    };
  }
  ...
  if (!(flatBright > flatDark + 20)) {
    return {
      ok: false,
      reason: `material contrast collapsed (dark ${flatDark.toFixed(1)} vs bright ${flatBright.toFixed(1)})`,
    };
  }
  return meanLuma(after) > 1 ? { ok: true } : { ok: false, reason: "SEM output is black" };
};
...
export const runRadarSweepPersistence = (): Result => {
  ...
  const r = 0.5;
  const behind = patch(early, 65, r),
    ahead = patch(early, 115, r);
  if (!(behind > ahead * 1.5)) {
    return {
      ok: false,
      reason: `no persistence trail behind the sweep (behind ${behind.toFixed(1)} vs ahead ${ahead.toFixed(1)} at bearing 90)`,
    };
  }
  const staleThen = patch(early, 155, r),
    freshNow = patch(later, 155, r);
  if (!(freshNow > staleThen * 1.5)) {
    return {
      ok: false,
      reason: `sweep did not rotate with _frameIndex (bearing 155: ${staleThen.toFixed(1)} at frame 3 -> ${freshNow.toFixed(1)} at frame 6)`,
    };
  }
  const wasFresh = patch(early, 65, r),
    nowFading = patch(later, 65, r);
  return nowFading < wasFresh
    ? { ok: true }
    : {
        ok: false,
        reason: `trail at bearing 65 did not fade as the sweep advanced (${wasFresh.toFixed(1)} -> ${nowFading.toFixed(1)})`,
      };
};
