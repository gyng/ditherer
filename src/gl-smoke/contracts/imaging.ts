...
export const runStainedGlassColorModes = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const webgl of [false, true]) {
    ...
    for (const colorMode of ["AVERAGE", "MEDIAN", "DOMINANT"]) {
      ...
      if (!pixels)
        return { ok: false, reason: `Stained glass ${webgl ? "GL" : "CPU"} readback failed` };
      let signature = 2166136261;
      for (let offset = 0; offset < pixels.length; offset += 4) {
        if (pixels[offset + 3] !== image.data[offset + 3]) {
          return {
            ok: false,
            reason: `Stained glass ${webgl ? "GL" : "CPU"} changed source alpha in ${colorMode}`,
          };
        }
        signature ^= pixels[offset];
        signature = Math.imul(signature, 16777619);
        signature ^= pixels[offset + 1];
        signature = Math.imul(signature, 16777619);
        signature ^= pixels[offset + 2];
        signature = Math.imul(signature, 16777619);
      }
    ...
    if (signatures.size !== 3) {
      return {
        ok: false,
        reason: `Stained glass ${webgl ? "GL" : "CPU"} color modes are not all live`,
      };
    }
    ...
    const customPixels = customOutput
      .getContext("2d", { willReadFrequently: true })
      ?.getImageData(0, 0, width, height).data;
    if (!customPixels)
      return {
        ok: false,
        reason: `Stained glass ${webgl ? "GL" : "CPU"} custom-palette readback failed`,
      };
    let darkLeadingPixels = 0;
    for (let offset = 0; offset < customPixels.length; offset += 4) {
      if (
        customPixels[offset] < 40 &&
        customPixels[offset + 1] < 40 &&
        customPixels[offset + 2] < 40
      ) {
        darkLeadingPixels++;
      }
    }
    if (darkLeadingPixels < width) {
      return {
        ok: false,
        reason: `Stained glass ${webgl ? "GL" : "CPU"} custom palette recolored its lead network`,
      };
    }
  }
  return { ok: true };
...
const makeSolidColorCanvas = (
  ...
  for (let offset = 0; offset < image.data.length; offset += 4) {
    image.data[offset] = rgba[0] ?? 0;
    image.data[offset + 1] = rgba[1] ?? 0;
    image.data[offset + 2] = rgba[2] ?? 0;
...
export const runScreenToneContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const cellSize of [8, 16, 48]) {
    ...
    if (means[0] < 0.98 || means[2] > 0.02 || Math.abs(means[1] - 0.5) > 0.09) {
      return {
        ok: false,
        reason: `Halftone Line tone response failed at pitch ${cellSize}: ${means.map((v) => v.toFixed(3)).join(",")}`,
      };
    }
  }
  ...
  for (const value of [64, 128, 192, 255]) {
    ...
    if (
      Math.abs(meanChannel(pixels, 0) - value / 255) > 0.08 ||
      meanChannel(pixels, 1) > 0.01 ||
      meanChannel(pixels, 2) > 0.01
    ) {
  ...
  const registrationRenders = [0, 3].map((offsetR) =>
    canvasPixels(
      color.func(registrationSource, {
        ...(color.defaults ?? {}),
        dotSize: 16,
        offsetR,
        offsetG: 0,
  ...
  if (!registrationRenders[0] || !registrationRenders[1])
    return { ok: false, reason: "Color Halftone registration readback failed" };
  ...
  if (registrationChanges < 96)
    return { ok: false, reason: "Color Halftone registration does not move its plate geometry" };

  const alphaSource = document.createElement("canvas");
  alphaSource.width = 64;
  alphaSource.height = 48;
  ...
  for (let i = 0; i < alphaImage.data.length; i += 4) {
    alphaImage.data[i] = 160;
    alphaImage.data[i + 1] = 90;
    alphaImage.data[i + 2] = 210;
    alphaImage.data[i + 3] = ((i / 4) * 37) & 255;
  }
  alphaContext.putImageData(alphaImage, 0, 0);
  for (const filter of [line, color]) {
    const output = filter.func(alphaSource, {
      ...(filter.defaults ?? {}),
      palette,
      _webglAcceleration: true,
    }) as HTMLCanvasElement;
    ...
    for (let i = 3; i < pixels.length; i += 4) {
      if (pixels[i] !== alphaImage.data[i])
        return { ok: false, reason: `${filter.name} changed source alpha` };
    }
  }
  return { ok: true };
};
...
export const runJpegSubsamplingModes = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const subsampling of ["444", "422", "420"]) {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
      if (pixels[i + 3] !== image.data[i + 3])
        return { ok: false, reason: `JPEG ${subsampling} changed alpha` };
      hash ^= pixels[i];
  ...
  if (signatures.size !== 3)
    return { ok: false, reason: "JPEG subsampling modes are not all live" };
  ...
  return high - low <= 3
    ? { ok: true }
    : { ok: false, reason: `JPEG partial 8x8 edge drifted by ${high - low} levels` };
};
...
export const runLegacyFieldContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!contour || !diffusion)
    return { ok: false, reason: "legacy field filters missing from registry" };
  const palette = { ...nearest, options: { ...nearest.options, levels: 256 } };
  ...
  for (let y = 0; y < 8; y += 1)
    for (let x = 0; x < 256; x += 1) {
      ...
      rampImage.data[offset] = x;
      rampImage.data[offset + 1] = x;
      rampImage.data[offset + 2] = x;
      rampImage.data[offset + 3] = 255;
    }
  rampContext.putImageData(rampImage, 0, 0);
  const rampInput = canvasPixels(ramp);
  const filled = canvasPixels(
    contour.func(ramp, {
      ...(contour.defaults ?? {}),
      levels: 5,
      fillMode: "FILLED",
  ...
  for (let offset = 0; offset < filled.length; offset += 4) {
    ...
    if (filled[offset + 3] !== rampInput[offset + 3])
      return { ok: false, reason: "Contour Lines changed source alpha" };
  }
  if (tones.size !== 5)
    return { ok: false, reason: `Contour filled mode produced ${tones.size} tones instead of 5` };
  ...
  for (let y = 0; y < 32; y += 1)
    for (let x = 0; x < 32; x += 1) {
      ...
      checkerImage.data[offset] = value;
      checkerImage.data[offset + 1] = value;
      checkerImage.data[offset + 2] = value;
      checkerImage.data[offset + 3] = 255;
    }
  checkerContext.putImageData(checkerImage, 0, 0);
  const diffused = canvasPixels(
    diffusion.func(checker, {
      ...(diffusion.defaults ?? {}),
      iterations: 10,
      kappa: 200,
      lambda: 0.2,
  ...
  for (let offset = 0; offset < diffused.length; offset += 4) {
    min = Math.min(min, diffused[offset]);
    max = Math.max(max, diffused[offset]);
    if (diffused[offset + 3] !== 255)
      return { ok: false, reason: "Anisotropic Diffusion changed opaque source alpha" };
  }
  if (max - min > 1)
    return {
      ok: false,
      reason: `Anisotropic Diffusion low-noise checker did not converge (range ${min}..${max})`,
    };
  ...
  for (let offset = 3; offset < alphaImage.data.length; offset += 4)
    alphaImage.data[offset] = ((offset / 4) * 37) & 255;
  alphaContext.putImageData(alphaImage, 0, 0);
  for (const filter of [contour, diffusion]) {
    const output = canvasPixels(
      filter.func(alphaSource, {
        ...(filter.defaults ?? {}),
        palette,
        _webglAcceleration: true,
      }) as HTMLCanvasElement,
    ...
    for (let offset = 3; offset < output.length; offset += 4) {
      if (output[offset] !== alphaImage.data[offset])
        return { ok: false, reason: `${filter.name} changed source alpha` };
    }
  }
  return { ok: true };
...
export const runLegacyPrinterAndOutlineContracts = ():
  | { ok: true }
  | { ok: false; reason: string } => {
  const dot = filterIndex["Dot Matrix"];
  const outline = filterIndex["Pixel Outline"];
  if (!dot || !outline)
    return { ok: false, reason: "legacy printer/outline filters missing from registry" };
  const palette = { ...nearest, options: { ...nearest.options, levels: 256 } };
  ...
  for (let toneIndex = 0; toneIndex < expectedFractions.length; toneIndex += 1) {
    ...
    let fired = 0;
    let cells = 0;
    for (let cy = 0; cy < 96; cy += 6)
      for (let cx = 0; cx < 96; cx += 6) {
    ...
    if (Math.abs(fraction - darkness) > 0.07) {
      return {
        ok: false,
        reason: `Dot Matrix strike density ${fraction.toFixed(3)} did not track ${darkness}`,
      };
    }
  }

  const alphaSource = document.createElement("canvas");
  alphaSource.width = 48;
  alphaSource.height = 32;
  const alphaContext = alphaSource.getContext("2d");
    ...
    for (const webgl of [false, true]) {
      ...
      for (let offset = 3; offset < output.length; offset += 4) {
        if (output[offset] !== alphaImage.data[offset])
          return {
            ok: false,
            reason: `${filter.name} ${webgl ? "GL" : "CPU"} changed source alpha`,
          };
      }
    }
  }
...
export const runCoreToneEdgeContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!posterize || !cmyk || !edge || !clahe)
    return { ok: false, reason: "core tone/edge filters missing" };
  const palette = { ...nearest, options: { ...nearest.options, levels: 256 } };

  for (const value of [64, 128, 191]) {
    const output = canvasPixels(
      posterize.func(makeSolidCanvas(64, 64, value), {
        ...(posterize.defaults ?? {}),
        levelsR: 2,
        levelsG: 2,
        levelsB: 2,
    ...
    if (Math.abs(mean - value / 255) > 0.02) {
      return {
        ok: false,
        reason: `Posterize Dither mean ${mean.toFixed(3)} did not preserve ${value / 255}`,
      };
    }
  }
  ...
  for (const pitch of [2, 6, 12]) {
    ...
    if (
      Math.abs(red - 128 / 255) > 0.08 ||
      meanChannel(output, 1) < 0.98 ||
      meanChannel(output, 2) < 0.98
    ) {
      return {
        ok: false,
        reason: `CMYK Halftone pitch ${pitch} area response was ${red.toFixed(3)}`,
      };
    }
  }
  const periodicSource = makeSolidColorCanvas(94, 70, [128, 255, 255, 255]);
  const periodic = [0, 180].map((angle) =>
    canvasPixels(
      cmyk.func(periodicSource, {
        ...(cmyk.defaults ?? {}),
        dotSize: 6,
        angleC: angle,
        angleM: 0,
  ...
  if (!periodic[0] || !periodic[1])
    return { ok: false, reason: "CMYK angle-periodicity readback failed" };
  let periodicChanges = 0;
  for (let i = 0; i < periodic[0].length; i += 1)
    if (periodic[0][i] !== periodic[1][i]) periodicChanges += 1;
  if (periodicChanges > periodic[0].length * 0.01)
    return { ok: false, reason: `CMYK 0/180 screens differed in ${periodicChanges} channels` };
  ...
  const mixedOutput = canvasPixels(
    cmyk.func(mixed, {
      ...(cmyk.defaults ?? {}),
      dotSize: 8,
      palette,
  ...
  for (let offset = 0; offset < mixedOutput.length; offset += 4) {
    if (
      Math.max(mixedOutput[offset], mixedOutput[offset + 1], mixedOutput[offset + 2]) -
        Math.min(mixedOutput[offset], mixedOutput[offset + 1], mixedOutput[offset + 2]) >
      32
    )
      chromaticPixels += 1;
  }
  if (chromaticPixels < 500)
    return {
      ok: false,
      reason: "CMYK separated the averaged RGB cell instead of averaging plate coverages",
    };
  ...
  for (let y = 0; y < 32; y += 1)
    for (let x = 0; x < 64; x += 1) {
      ...
      sigmoidImage.data[offset] = value;
      sigmoidImage.data[offset + 1] = value;
      sigmoidImage.data[offset + 2] = value;
      sigmoidImage.data[offset + 3] = 255;
    }
  sigmoidContext.putImageData(sigmoidImage, 0, 0);
  const traced = canvasPixels(
    edge.func(sigmoid, {
      ...(edge.defaults ?? {}),
      threshold: 10,
      lineWidth: 1,
  ...
  if (darkColumns.size > 6)
    return { ok: false, reason: `Edge Trace NMS retained ${darkColumns.size} gradient columns` };
  const alphaSource = document.createElement("canvas");
  alphaSource.width = 64;
  alphaSource.height = 32;
  const alphaContext = alphaSource.getContext("2d");
  ...
  for (let offset = 3; offset < alphaImage.data.length; offset += 4)
    alphaImage.data[offset] = ((offset / 4) * 37) & 255;
  alphaContext.putImageData(alphaImage, 0, 0);
  const identity = canvasPixels(
    edge.func(alphaSource, {
      ...(edge.defaults ?? {}),
      renderMode: "OVERLAY",
      overlayMix: 0,
      palette,
  ...
  if (!identity || !alphaPixels)
    return { ok: false, reason: "Edge Trace identity readback failed" };
  for (let i = 0; i < identity.length; i += 1) {
    if (identity[i] !== alphaPixels[i])
      return { ok: false, reason: "Edge Trace overlay mix zero is not RGBA identity" };
  }
  const paletteIdentity = canvasPixels(
    edge.func(alphaSource, {
      ...(edge.defaults ?? {}),
      renderMode: "OVERLAY",
      overlayMix: 0,
      palette: { ...nearest, options: { ...nearest.options, levels: 2 } },
      _webglAcceleration: true,
    }) as HTMLCanvasElement,
  );
  ...
  for (let i = 0; i < paletteIdentity.length; i += 1) {
    if (paletteIdentity[i] !== alphaPixels[i])
      return { ok: false, reason: "Edge Trace zero mix applied a nonidentity palette" };
  }

  const binaryPalette = { ...nearest, options: { ...nearest.options, levels: 2 } };
  const claheOutput = canvasPixels(
    clahe.func(alphaSource, {
      ...(clahe.defaults ?? {}),
      tileSize: 8,
      clipLimit: 3,
  ...
  for (let offset = 0; offset < claheOutput.length; offset += 4) {
    ...
    if (claheOutput[offset + 3] !== alphaImage.data[offset + 3])
      return { ok: false, reason: "CLAHE changed source alpha" };
  }
  ...
  for (let y = 0; y < 8; y += 1)
    for (let x = 0; x < multirow.width; x += 1) {
      const offset = (y * multirow.width + x) * 4;
      const value = (Math.floor(x / 8) * 29 + (x & 7) * 17 + y * 11) & 255;
      multirowImage.data[offset] = value;
      multirowImage.data[offset + 1] = value;
      multirowImage.data[offset + 2] = value;
      multirowImage.data[offset + 3] = 255;
    }
  multirowContext.putImageData(multirowImage, 0, 0);
  const multirowOutputs = [false, true].map((webgl) =>
    canvasPixels(
      clahe.func(multirow, {
        ...(clahe.defaults ?? {}),
        tileSize: 8,
  ...
  if (!multirowOutputs[0] || !multirowOutputs[1])
    return { ok: false, reason: "CLAHE multi-row CDF atlas failed" };
  let atlasDelta = 0;
  ...
  if (atlasDelta > 1)
    return { ok: false, reason: `CLAHE second atlas row disagreed with CPU by ${atlasDelta}` };
  ...
  if (!edgeWidths[0] || !edgeWidths[1])
    return { ok: false, reason: "CLAHE partial-tile readback failed" };
  let partialTileDelta = 0;
  for (let y = 0; y < 16; y += 1)
    for (let x = 0; x < 32; x += 1) {
      const left = (y * 32 + x) * 4;
      const right = (y * 33 + x) * 4;
      partialTileDelta = Math.max(
        partialTileDelta,
  ...
  if (partialTileDelta > 3)
    return {
      ok: false,
      reason: `CLAHE one-pixel edge tile changed overlap by ${partialTileDelta}`,
    };
  ...
  for (const filter of [edge, clahe, cmyk]) {
    const renders = hiddenFixtures.map((source) =>
      canvasPixels(
        filter.func(source, {
          ...(filter.defaults ?? {}),
          tileSize: 8,
    ...
    if (!renders[0] || !renders[1])
      return { ok: false, reason: `${filter.name} hidden-RGB readback failed` };
    for (let y = 0; y < 16; y += 1)
      for (let x = 16; x < 32; x += 1) {
        const offset = (y * 32 + x) * 4;
        for (let channel = 0; channel < 3; channel += 1) {
          if (renders[0][offset + channel] !== renders[1][offset + channel])
            return { ok: false, reason: `${filter.name} visible output depends on hidden RGB` };
        }
      }
    if (filter === edge) {
      let silhouetteInk = 0;
...
export const runUpgradedAlphaPreservation = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const [name, override] of cases) {
    ...
    try {
      ...
      return {
        ok: false,
        reason: `${name} alpha render threw: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
    const pixels = canvasPixels(output);
...
export const runLegacyGpuStylizerContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!lens || !popArt || !facet)
    return { ok: false, reason: "legacy stylizer registry entry missing" };
  ...
  for (const [name, filter] of [
    ...
    try {
      ...
      return {
        ok: false,
        reason: `${name} sparse-state render threw: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
    if (sparseOutput.width !== patterned.width || sparseOutput.height !== patterned.height) {
      return {
        ok: false,
        reason: `${name} sparse-state size=${sparseOutput.width}x${sparseOutput.height}`,
      };
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: `Lens Flare identity render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  for (let index = 0; index < inactiveLens.length; index += 1) {
    if (inactiveLens[index] !== sourceImage.data[index]) {
      return {
        ok: false,
        reason: `Lens Flare intensity=0 changed byte ${index}: ${sourceImage.data[index]} -> ${inactiveLens[index]}`,
      };
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: `Pop Art tone render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  for (const fillMode of ["AVERAGE", "CENTER"]) {
    ...
    try {
      ...
      return {
        ok: false,
        reason: `Facet ${fillMode} transparent render threw: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
    ...
    for (let index = 3; index < output.length; index += 4) {
      if (output[index] !== 0) {
        return {
          ok: false,
          reason: `Facet ${fillMode} made transparent pixel ${index / 4} alpha=${output[index]}`,
        };
      }
    }
  }
...
export const runPainterlyStylizerContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!pencil || !mosaic || !oil)
    return { ok: false, reason: "painterly stylizer registry entry missing" };
  const identityPalette = { ...nearest, options: { ...nearest.options, levels: 256 } };
  ...
  for (const [name, filter, backends] of [
    ...
    for (const webgl of backends) {
      ...
      try {
        ...
        return {
          ok: false,
          reason: `${name} ${webgl ? "GL" : "CPU"} alpha render threw: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
      if (!pixels) return { ok: false, reason: `${name} alpha readback failed` };
      for (let offset = 3; offset < pixels.length; offset += 4) {
        if (pixels[offset] !== image.data[offset]) {
          return {
            ok: false,
            reason: `${name} ${webgl ? "GL" : "CPU"} changed alpha at ${offset / 4}`,
          };
        }
      }
    }
  ...
  for (const [name, filter] of [
    ...
    const cpu = canvasPixels(
      filter.func(opaque, { ...base, _webglAcceleration: false }) as HTMLCanvasElement,
    );
    const gpu = canvasPixels(
      filter.func(opaque, { ...base, _webglAcceleration: true }) as HTMLCanvasElement,
    );
    ...
    if (meanDelta > 1 || maxDelta > 40) {
      return {
        ok: false,
        reason: `${name} CPU/GL delta mean=${meanDelta.toFixed(3)} max=${maxDelta}`,
      };
    }
  }
  ...
  for (const [name, filter, webgl] of [
    ["Pencil sketch CPU", pencil, false],
    ["Pencil sketch GL", pencil, true],
    ["Mosaic tile CPU", mosaic, false],
    ["Mosaic tile GL", mosaic, true],
    ["Oil painting", oil, true],
  ] as const) {
    const options = {
      ...(filter.defaults ?? {}),
  ...
  for (const webgl of [false, true]) {
    ...
    if (
      transitions.some((count, index) => count < 0 || (index > 0 && count < transitions[index - 1]))
    ) {
      return {
        ok: false,
        reason: `Pencil ${webgl ? "GL" : "CPU"} density is not monotonic: ${transitions.join(",")}`,
      };
    }
  }
  ...
  edgeFlowContext.fillStyle = "rgb(40,40,40)";
  edgeFlowContext.fillRect(0, 0, 32, 64);
  edgeFlowContext.fillStyle = "rgb(210,210,210)";
  edgeFlowContext.fillRect(32, 0, 32, 64);
  const edgeFlowPixels = canvasPixels(
    pencil.func(edgeFlow, {
      ...(pencil.defaults ?? {}),
      ...runtimeOptions(),
      palette: identityPalette,
      strokeDensity: 6,
  ...
  if (tangentDelta > normalDelta * 0.25) {
    return {
      ok: false,
      reason: `Pencil strokes cross edge flow: tangent=${tangentDelta}, normal=${normalDelta}`,
    };
  }
  ...
  tileImage.data[outlier] = 255;
  tileImage.data[outlier + 1] = 255;
  tileImage.data[outlier + 2] = 255;
  ...
  const mosaicCpu = canvasPixels(
    mosaic.func(tileFixture, { ...tileOptions, _webglAcceleration: false }) as HTMLCanvasElement,
  );
  const mosaicGpu = canvasPixels(
    mosaic.func(tileFixture, { ...tileOptions, _webglAcceleration: true }) as HTMLCanvasElement,
  );
  if (
    !mosaicCpu ||
    !mosaicGpu ||
    mosaicCpu[0] < 14 ||
    mosaicCpu[0] > 18 ||
    mosaicGpu[0] !== mosaicCpu[0]
  ) {
    return {
      ok: false,
      reason: `Mosaic sampled mean mismatch CPU=${mosaicCpu?.[0]} GL=${mosaicGpu?.[0]}`,
    };
  }

  const sparseTile = document.createElement("canvas");
  sparseTile.width = 40;
  sparseTile.height = 40;
  const sparseTileContext = sparseTile.getContext("2d");
  if (!sparseTileContext)
    return { ok: false, reason: "Mosaic sparse-tile fixture has no 2d context" };
  ...
  for (const webgl of [false, true]) {
    ...
    if (
      !first ||
      !second ||
      first[sparseFeature] < 210 ||
      first[sparseFeature + 1] > 40 ||
      first.some((value, index) => value !== second[index])
    ) {
      return {
        ok: false,
        reason: `Mosaic ${webgl ? "GL" : "CPU"} lost or destabilized a sparse opaque feature`,
      };
    }
  }
  ...
  const partialCpu = canvasPixels(
    mosaic.func(source, { ...partialOptions, _webglAcceleration: false }) as HTMLCanvasElement,
  );
  const partialGpu = canvasPixels(
    mosaic.func(source, { ...partialOptions, _webglAcceleration: true }) as HTMLCanvasElement,
  );
  if (!partialCpu || !partialGpu)
    return { ok: false, reason: "Mosaic partial-alpha parity readback failed" };
  let partialMaxDelta = 0;
  for (let offset = 0; offset < partialCpu.length; offset += 4) {
    if (image.data[offset + 3] === 0) continue;
    partialMaxDelta = Math.max(
      partialMaxDelta,
  ...
  if (partialMaxDelta > 2)
    return { ok: false, reason: `Mosaic partial-alpha CPU/GL max delta=${partialMaxDelta}` };
  ...
  for (const [x, y, value] of [
    ...
    tieImage.data[offset] = value;
    tieImage.data[offset + 1] = value;
    tieImage.data[offset + 2] = value;
  }
  tieContext.putImageData(tieImage, 0, 0);
  const oilPixels = canvasPixels(
    oil.func(tieFixture, {
      ...(oil.defaults ?? {}),
      ...runtimeOptions(),
      palette: identityPalette,
  ...
  if (!oilPixels || oilPixels[tieCenter] < 240) {
    return {
      ok: false,
      reason: `Oil modal tie did not prefer center-nearest bin: ${oilPixels?.[tieCenter]}`,
    };
  }
  ...
  const partialTiePixels = canvasPixels(
    oil.func(tieFixture, {
      ...(oil.defaults ?? {}),
      ...runtimeOptions(),
      palette: identityPalette,
  ...
  if (!partialTiePixels || partialTiePixels[tieCenter] < 240) {
    return {
      ok: false,
      reason: `Oil partial-alpha tie was numerically biased: ${partialTiePixels?.[tieCenter]}`,
    };
  }
...
export const runBilateralBlurContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const workingResolution of ["FULL", "HALF", "QUARTER"]) {
    for (const linearize of [false, true]) {
      ...
      if (!cpu || !gpu)
        return {
          ok: false,
          reason: `Bilateral ${workingResolution} ${linearize ? "linear" : "sRGB"} parity readback failed`,
        };
      let totalDelta = 0;
      let maxDelta = 0;
      let samples = 0;
      ...
      if (meanDelta > 1.5 || maxDelta > 8) {
        return {
          ok: false,
          reason: `Bilateral ${workingResolution} parity delta mean=${meanDelta.toFixed(3)} max=${maxDelta}`,
        };
      }
    }
  }
  ...
  for (let offset = 0; offset < hiddenImage.data.length; offset += 4) {
    if (hiddenImage.data[offset + 3] === 0) {
      hiddenImage.data[offset] = 255;
      hiddenImage.data[offset + 1] = 0;
      hiddenImage.data[offset + 2] = 255;
    }
  }
  hiddenContext.putImageData(hiddenImage, 0, 0);
  for (const webgl of [false, true]) {
    ...
    if (!first || !second)
      return { ok: false, reason: `Bilateral ${webgl ? "GL" : "CPU"} hidden-RGB readback failed` };
    for (let offset = 0; offset < first.length; offset += 4) {
      if (first[offset + 3] !== image.data[offset + 3]) {
        return {
          ok: false,
          reason: `Bilateral ${webgl ? "GL" : "CPU"} changed alpha at ${offset / 4}`,
        };
      }
      if (image.data[offset + 3] === 0) {
        if (first[offset] !== 0 || first[offset + 1] !== 0 || first[offset + 2] !== 0) {
          return {
            ok: false,
            reason: `Bilateral ${webgl ? "GL" : "CPU"} did not canonicalize transparent RGB`,
          };
        }
      } else if (
        first[offset] !== second[offset] ||
        ...
        return {
          ok: false,
          reason: `Bilateral ${webgl ? "GL" : "CPU"} leaked hidden RGB at ${offset / 4}`,
        };
      }
    }
  }
  ...
  for (const webgl of [false, true]) {
    ...
    if (!identity || !pixels)
      return {
        ok: false,
        reason: `Bilateral ${webgl ? "GL" : "CPU"} custom-palette readback failed`,
      };
    ...
    for (let offset = 0; offset < pixels.length; offset += 4) {
      if (
        ...
        return {
          ok: false,
          reason: `Bilateral ${webgl ? "GL" : "CPU"} palette repopulated transparent RGB`,
        };
      }
      if (
        image.data[offset + 3] > 0 &&
        (pixels[offset] !== identity[offset] ||
          pixels[offset + 1] !== identity[offset + 1] ||
          pixels[offset + 2] !== identity[offset + 2])
      )
        recoloredVisible += 1;
    }
    if (recoloredVisible < source.width * source.height * 0.5) {
      return {
        ok: false,
        reason: `Bilateral ${webgl ? "GL" : "CPU"} custom palette did not recolor visible output`,
      };
    }
  }

  const sparseAlpha = document.createElement("canvas");
  sparseAlpha.width = 40;
  sparseAlpha.height = 40;
  const sparseAlphaContext = sparseAlpha.getContext("2d");
  if (!sparseAlphaContext)
    return { ok: false, reason: "Bilateral sparse-alpha fixture unavailable" };
  ...
  for (const webgl of [false, true]) {
    ...
    if (!pixels)
      return {
        ok: false,
        reason: `Bilateral ${webgl ? "GL" : "CPU"} sparse-alpha readback failed`,
      };
    for (const [x, y, alpha] of [
      [13, 17, 1],
      [29, 9, 2],
      ...
      if (pixels[offset + 3] !== alpha || pixels[offset] < 100) {
        return {
          ok: false,
          reason: `Bilateral ${webgl ? "GL" : "CPU"} lost alpha-${alpha} detail: ${pixels[offset]}/${pixels[offset + 3]}`,
        };
      }
    }
  }
  ...
  opaqueNoise.width = 64;
  opaqueNoise.height = 32;
  ...
  for (let y = 0; y < 32; y += 1)
    for (let x = 0; x < 64; x += 1) {
      ...
      noiseImage.data[offset] = value;
      noiseImage.data[offset + 1] = value;
      noiseImage.data[offset + 2] = value;
  ...
  const narrow = render(
    opaqueNoise,
    { workingResolution: "FULL", sigmaSpatial: 1, sigmaRange: 100 },
    true,
  );
  const wide = render(
    opaqueNoise,
    { workingResolution: "FULL", sigmaSpatial: 10, sigmaRange: 100 },
    true,
  );
  if (!narrow || !wide || variance(wide) >= variance(narrow) * 0.7) {
    return {
      ok: false,
      reason: `Bilateral spatial smoothing not live: ${narrow ? variance(narrow).toFixed(2) : "null"} -> ${wide ? variance(wide).toFixed(2) : "null"}`,
    };
  }
  ...
  edgeContext.fillStyle = "rgb(80,80,80)";
  edgeContext.fillRect(0, 0, 32, 32);
  edgeContext.fillStyle = "rgb(180,180,180)";
  edgeContext.fillRect(32, 0, 32, 32);
  const edgeTight = render(
    edge,
    { workingResolution: "FULL", sigmaSpatial: 10, sigmaRange: 5 },
    true,
  );
  const edgeLoose = render(
    edge,
    { workingResolution: "FULL", sigmaSpatial: 10, sigmaRange: 100 },
    true,
  );
  ...
  if (
    !edgeTight ||
    !edgeLoose ||
    edgeLoose[boundary] <= edgeTight[boundary] + 2 ||
    edgeTight[boundary] > 82 ||
    edgeTight[boundaryRight] < 178
  ) {
    return {
      ok: false,
      reason: `Bilateral range control not live: ${edgeTight?.[boundary]} -> ${edgeLoose?.[boundary]}`,
    };
  }
  ...
  for (const workingResolution of ["FULL", "HALF", "QUARTER"]) {
    const pixels = render(
      opaqueNoise,
      { workingResolution, sigmaSpatial: 8, sigmaRange: 40 },
      true,
    );
    if (!pixels)
      return { ok: false, reason: `Bilateral ${workingResolution} control readback failed` };
    let hash = 2166136261;
    for (let offset = 0; offset < pixels.length; offset += 4) {
      hash ^= pixels[offset];
      hash = Math.imul(hash, 16777619);
  ...
  if (signatures.size !== 3)
    return { ok: false, reason: "Bilateral working-resolution modes are not all live" };

  const lowContrastEdge = document.createElement("canvas");
  lowContrastEdge.width = 96;
  lowContrastEdge.height = 32;
  ...
  lowContrastContext.fillStyle = "rgb(100,100,100)";
  lowContrastContext.fillRect(0, 0, 48, 32);
  lowContrastContext.fillStyle = "rgb(140,140,140)";
  lowContrastContext.fillRect(48, 0, 48, 32);
  ...
  for (const workingResolution of ["FULL", "HALF", "QUARTER"]) {
    ...
    if (!pixels)
      return { ok: false, reason: `Bilateral ${workingResolution} scale readback failed` };
  ...
  if (
    ...
    return {
      ok: false,
      reason: `Bilateral working scale drift centers=${centers.join("/")} widths=${widths.join("/")}`,
    };
  }

  const linearOff = render(
    opaqueNoise,
    { workingResolution: "HALF", sigmaSpatial: 6, sigmaRange: 100, _linearize: false },
    true,
  );
  const linearOn = render(
    opaqueNoise,
    { workingResolution: "HALF", sigmaSpatial: 6, sigmaRange: 100, _linearize: true },
    true,
  );
  if (
    !linearOff ||
    !linearOn ||
    !linearOff.some((value, index) => index % 4 !== 3 && value !== linearOn[index])
  ) {
  ...
  for (const [width, height] of [
    ...
    const pixels = render(
      tiny,
      { workingResolution: "QUARTER", sigmaSpatial: 12, sigmaRange: 1 },
      true,
    );
    if (
      !pixels ||
      pixels.some((value, index) => (index % 4 !== 3 ? Math.abs(value - 123) > 1 : value !== 255))
    ) {
...
export const runEinkDisplayContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const mode of ["GRAYSCALE", "COLOR"]) {
    ...
    if (!gpu || !cpu || gpu.length !== cpu.length)
      return { ok: false, reason: `E-ink ${mode} CPU/GL readback failed` };
    let totalDelta = 0;
    ...
    if (meanDelta > 1)
      return { ok: false, reason: `E-ink ${mode} CPU/GL mean delta=${meanDelta.toFixed(3)}` };
  }
  ...
  if (
    ...
    return {
      ok: false,
      reason: `E-ink levels gray=${grayLevels.size} [${sortedGray[0]}..${highestGray}], color=${channelLevels.map((levels) => levels.size).join("/")}`,
    };
  }
    ...
    for (let cellX = 0; cellX + 2 < width; cellX += 3) {
      ...
      for (let dy = 0; dy < 3; dy += 1) {
        for (let dx = 0; dx < 3; dx += 1) {
          ...
          if (
            color[offset] !== color[reference] ||
            color[offset + 1] !== color[reference + 1] ||
            color[offset + 2] !== color[reference + 2]
          ) {
            return {
              ok: false,
              reason: `E-ink Kaleido cell incoherent at ${cellX + dx},${cellY + dy}`,
            };
          }
        }
      }
  ...
  if (!partialClean || !partialResidual || !fullClean || !fullPrevious)
    return { ok: false, reason: "E-ink refresh render failed" };
  let partialChanges = 0;
  let fullChanges = 0;
  for (let index = 0; index < partialClean.length; index += 4) {
    if (
      partialResidual[index] !== partialResidual[index + 1] ||
      partialResidual[index] !== partialResidual[index + 2]
    ) {
      return {
        ok: false,
        reason: `E-ink monochrome residual carried chroma at pixel ${index / 4}`,
      };
    }
    if (
      partialClean[index] !== partialResidual[index] ||
      partialClean[index + 1] !== partialResidual[index + 1] ||
      partialClean[index + 2] !== partialResidual[index + 2]
    )
      partialChanges += 1;
    if (
      fullClean[index] !== fullPrevious[index] ||
      fullClean[index + 1] !== fullPrevious[index + 1] ||
      fullClean[index + 2] !== fullPrevious[index + 2]
    )
      fullChanges += 1;
  }
  return partialChanges > width * height * 0.5 && fullChanges === 0
    ...
    : {
        ok: false,
        reason: `E-ink refresh residual partial=${partialChanges}, full=${fullChanges}`,
      };
};
...
export const runVintageTvSignalContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    lumaOutput = canvasPixels(
      filter.func(lumaSource, { ...base, chromaBandwidth: 8 }) as HTMLCanvasElement,
    );
    chromaOutput = canvasPixels(
      filter.func(chromaSource, { ...base, chromaBandwidth: 8 }) as HTMLCanvasElement,
    );
  } catch (error) {
    return {
      ok: false,
      reason: `Vintage TV bandwidth render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  const lumaInput = canvasPixels(lumaSource);
  const chromaInput = canvasPixels(chromaSource);
  if (!lumaInput || !chromaInput || !lumaOutput || !chromaOutput)
    return { ok: false, reason: "Vintage TV bandwidth readback failed" };
  const lumaRetention = stripeAmplitude(lumaOutput, 128, 32) / stripeAmplitude(lumaInput, 128, 32);
  const chromaRetention =
    stripeAmplitude(chromaOutput, 128, 32) / stripeAmplitude(chromaInput, 128, 32);
  if (lumaRetention < 0.8 || chromaRetention >= lumaRetention * 0.5) {
    return {
      ok: false,
      reason: `Vintage TV bandwidth retention luma=${lumaRetention.toFixed(3)}, chroma=${chromaRetention.toFixed(3)}`,
    };
  }
  ...
  const renderFrame = (frameIndex: number, verticalRoll: number): Uint8ClampedArray | null => {
    try {
      return canvasPixels(
        filter.func(makeGradientCanvas(96, 64), {
          ...base,
          chromaBandwidth: 0,
          frameIndex,
  ...
  if (!stillA || !stillB || !rolling)
    return { ok: false, reason: "Vintage TV roll readback failed" };
  let stillChanges = 0;
  let rollChanges = 0;
  for (let index = 0; index < stillA.length; index += 4) {
    if (
      stillA[index] !== stillB[index] ||
      stillA[index + 1] !== stillB[index + 1] ||
      stillA[index + 2] !== stillB[index + 2]
    )
      stillChanges += 1;
    if (
      stillA[index] !== rolling[index] ||
      stillA[index + 1] !== rolling[index + 1] ||
      stillA[index + 2] !== rolling[index + 2]
    )
      rollChanges += 1;
  }
  if (stillChanges !== 0 || rollChanges < 96 * 64 * 0.2) {
    return {
      ok: false,
      reason: `Vintage TV roll liveness still=${stillChanges}, rolling=${rollChanges}`,
    };
  }
  ...
  try {
    rasterOff = canvasPixels(
      filter.func(tall, { ...base, chromaBandwidth: 0, scanlineStrength: 0 }) as HTMLCanvasElement,
    );
    rasterOn = canvasPixels(
      filter.func(tall, {
        ...base,
        chromaBandwidth: 0,
  ...
  for (let index = 0; index < rasterOff.length; index += 4) {
    if (
      Math.abs(rasterOff[index] - rasterOn[index]) > 3 ||
      Math.abs(rasterOff[index + 1] - rasterOn[index + 1]) > 3 ||
      Math.abs(rasterOff[index + 2] - rasterOn[index + 2]) > 3
    )
      rasterChanges += 1;
  }
...
export const runDigicamFlashContracts = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const overrides of [
    ...
    try {
      ...
      return {
        ok: false,
        reason: `Digicam Flash parity threw: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
    ...
    const meanDelta = totalDelta / ((gpu.length / 4) * 3);
    if (meanDelta > 1.5)
      return { ok: false, reason: `Digicam Flash CPU/GL mean delta=${meanDelta.toFixed(3)}` };
  }

  const renderSolid = (flashPower: number, warmth: number): Uint8ClampedArray | null => {
    try {
      return canvasPixels(
        filter.func(makeSolidCanvas(64, 48, 96), {
          ...base,
          flashPower,
          warmth,
          ambient: 1,
  ...
  if (!noFlashWarm || !noFlashCool || !halfStop || !oneStop)
    return { ok: false, reason: "Digicam Flash exposure readback failed" };
  ...
  return !warmthChanged && mean(noFlashWarm) < mean(halfStop) && mean(halfStop) < mean(oneStop)
    ...
    : {
        ok: false,
        reason: `Digicam Flash separation warmth=${warmthChanged}, means=${mean(noFlashWarm).toFixed(1)}/${mean(halfStop).toFixed(1)}/${mean(oneStop).toFixed(1)}`,
      };
};

export const runNightVisionSignalDependentNoise = ():
  | { ok: true }
  | { ok: false; reason: string } => {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `night-vision noise render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  return darkDelta > 0.5 && brightDelta > darkDelta * 1.5
    ...
    : {
        ok: false,
        reason: `intensifier noise was not signal-dependent (${darkDelta.toFixed(2)} -> ${brightDelta.toFixed(2)})`,
      };
};
...
export const runAlphaStatisticsHiddenRgb = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const equivalentVisible = (
    ...
    if (!source || !first || !second)
      return { ok: false, reason: `${name} hidden-RGB readback failed` };
    for (let offset = 0; offset < first.length; offset += 4) {
      if (
        source[offset + 3] > 0 &&
        (first[offset] !== second[offset] ||
  ...
  const nightPair = pairedCanvases(32, 24, (x, y) =>
    x === 16 && y === 12 ? [0, 0, 0, 0] : [58 + (x % 5), 72 + (y % 7), 46, 255],
  );
  ...
  const ultrasoundPair = pairedCanvases(96, 80, (x, y) =>
    x < 48 && y > 12 ? [0, 0, 0, 0] : [56 + (x % 11), 72 + (y % 13), 48, 255],
  );
  if (!ultrasoundPair) return { ok: false, reason: "Ultrasound hidden-RGB fixture unavailable" };
  for (const webgl of [false, true]) {
    const result = equivalentVisible(
  ...
  const spectrogramPair = pairedCanvases(24, 32, (x, y) =>
    y === 8 ? [0, 0, 0, 0] : [80 + ((x * 7 + y * 11) % 80), 64 + ((x * 5 + y * 3) % 96), 48, 255],
  );
  if (!spectrogramPair) return { ok: false, reason: "Spectrogram hidden-RGB fixture unavailable" };
  for (const webgl of [false, true]) {
    const result = equivalentVisible(
      `Spectrogram ${webgl ? "GL" : "CPU"}`,
      spectrogram,
      spectrogramPair,
      {
  ...
  const lcdPair = pairedCanvases(24, 12, (x, y) =>
    y === 6 && (x === 6 || x === 18) ? [0, 0, 0, 0] : [72, 96, 120, 255],
  );
  ...
  const mavicaPair = pairedCanvases(64, 48, (x, y) =>
    x < 48
      ? [0, 0, 0, 0]
      : [
          72 + ((x * 7 + y * 5) % 112),
...
export const runLowAlphaStatisticsIsolation = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const compareOpaque = (
    ...
    const second = canvasPixels(filter.func(pair[1], options) as HTMLCanvasElement);
    if (!source || !first || !second)
      return { ok: false, reason: `${label} low-alpha readback failed` };
    let maximumDelta = 0;
    let maximumPixel = 0;
    for (let offset = 0; offset < first.length; offset += 4) {
    ...
    return maximumDelta <= tolerance
      ...
      : {
          ok: false,
          reason: `${label} low-alpha RGB changed opaque output by ${maximumDelta} at ${maximumPixel % pair[0].width},${Math.floor(maximumPixel / pair[0].width)}`,
        };
  };
  ...
  const mavicaCoverageZeroLow = makePair(64, 48, 0.75, 0, 32, [255, 8, 8], [255, 8, 8]);
  const mavicaCoverageZeroFull = makePair(64, 48, 0.75, 0, 255, [255, 8, 8], [255, 8, 8]);
  const mavicaStraightLowFull = makePair(48, 32, 1, 32, 255, [184, 112, 56], [184, 112, 56]);
  const mavicaStraightZeroLow = makePair(48, 32, 1, 0, 32, [184, 112, 56], [184, 112, 56]);
  if (
    !ultrasoundPair ||
    !mavicaLowColorPair ||
    !mavicaFullColorPair ||
    !mavicaCoverageZeroLow ||
    !mavicaCoverageZeroFull ||
    !mavicaStraightLowFull ||
    !mavicaStraightZeroLow
  ) {
  ...
  const render = (source: HTMLCanvasElement) =>
    canvasPixels(mavica.func(source, mavicaOptions) as HTMLCanvasElement);
  ...
  if (lowColor.meanDelta > fullColor.meanDelta * 0.35 + 0.5 || lowColor.maximumDelta > 96) {
    return {
      ok: false,
  ...
  if (
    lowCoverage.meanDelta > fullCoverage.meanDelta + 0.25 ||
  ...
  if (!(coverageZero[3] < coverageLow[3] && coverageLow[3] < coverageFull[3])) {
    return {
      ok: false,
      reason: "Mavica FD7 GL did not preserve monotonic alpha 0/32/255 coverage",
    };
  }
  ...
  const renderStraight = (source: HTMLCanvasElement) =>
    canvasPixels(mavica.func(source, straightOptions) as HTMLCanvasElement);
  ...
  for (let offset = 0; offset < straightLow.length; offset += 4) {
    if (
      straightZero[offset] !== 0 ||
      straightZero[offset + 1] !== 0 ||
      straightZero[offset + 2] !== 0 ||
      straightZero[offset + 3] !== 0
    ) {
      return {
        ok: false,
        reason: `Mavica FD7 GL alpha-zero output retained RGB at pixel ${offset / 4}`,
      };
    }
    if (straightLow[offset + 3] !== 32 || straightFull[offset + 3] !== 255) {
      return {
        ok: false,
        reason: `Mavica FD7 GL changed homogeneous alpha at pixel ${offset / 4}`,
      };
    }
  }
  ...
  if (
    hueDelta > 0.12 ||
    straightEnergyRatio < 0.55 ||
    straightEnergyRatio > 1.45 ||
...
export const runLcdLowAlphaEnergy = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const layout of ["STRIPE", "PENTILE", "DIAMOND"]) {
    ...
    if (highEnergy < 1000 || lowEnergy * 10 >= highEnergy) {
      return {
        ok: false,
        reason: `LCD ${layout} did not attenuate alpha-1 centre energy: ${lowEnergy}/${highEnergy}`,
      };
    }
  }
  return { ok: true };
...
export const runUltrasoundBoundaryEcho = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!boundaryContext)
    return { ok: false, reason: "ultrasound boundary fixture has no 2d context" };
  ...
  try {
    ...
    return {
      ok: false,
      reason: `ultrasound boundary render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  if (!uniformPixels || !boundaryPixels)
    return { ok: false, reason: "ultrasound boundary readback failed" };
  let uniformPeak = 0;
  let boundaryPeak = 0;
  ...
  return uniformPeak > 0 && boundaryPeak > uniformPeak * 2
    ...
    : {
        ok: false,
        reason: `impedance boundary did not dominate diffuse echo (${uniformPeak} -> ${boundaryPeak})`,
      };
};
...
export const runLenticularNeutralAndAngle = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    neutralPixels = canvasPixels(
      filter.func(neutral, { ...base, viewAngle: 0.35 }) as HTMLCanvasElement,
    );
  } catch (error) {
    return {
      ok: false,
      reason: `lenticular neutral render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: `lenticular angle render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
...
export const runLcdEmitterTopology = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const counts = (layout: string): [number, number, number] | null => {
    ...
    for (let index = 0; index < pixels.length; index += 4) {
      ...
      if (channels.filter((value, i) => i !== channel && value > 20).length === 0)
        result[channel] += 1;
    }
    return result;
  };
  ...
  if (!stripe || !pentile || !diamond)
    return { ok: false, reason: "LCD topology render/readback failed" };
  ...
  return stripeSpread <= 72 &&
    pentileChromaDelta <= 72 &&
    pentileBalance <= 144 &&
    diamondGreenDense
    ? { ok: true }
    : {
        ok: false,
        reason: `LCD topology counts stripe=${stripe} pentile=${pentile} diamond=${diamond}`,
      };
};
...
export const runSpectrogramSharedMagnitude = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const value = Math.round(
        127.5 + 127.5 * amplitude * Math.sin((2 * Math.PI * 8 * y) / height),
      );
      image.data[offset] = value;
      image.data[offset + 1] = value;
      image.data[offset + 2] = value;
  ...
  try {
    ...
    return {
      ok: false,
      reason: `spectrogram magnitude render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  return highLevel > lowLevel + 30
    ...
    : {
        ok: false,
        reason: `spectrogram self-normalized target-bin energy (${lowLevel.toFixed(1)} vs ${highLevel.toFixed(1)})`,
      };
};
...
export const runSpectrogramTinyNyquist = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `tiny spectrogram render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  for (const mode of ["RED_CYAN", "RED_GREEN", "MAGENTA_GREEN", "YELLOW_BLUE"]) {
    for (const depthSource of ["LUMINANCE", "EDGE", "CONSTANT"]) {
      ...
      try {
        ...
        cpu = canvasPixels(
          filter.func(source, { ...options, _webglAcceleration: false }) as HTMLCanvasElement,
        );
      } catch (error) {
        return {
          ok: false,
          reason: `anaglyph ${mode}/${depthSource} parity threw: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
      if (!gpu || !cpu || gpu.length !== cpu.length) {
      ...
      if (meanDelta > 2) {
        return {
          ok: false,
          reason: `anaglyph ${mode}/${depthSource} CPU/GL mean delta=${meanDelta.toFixed(3)}`,
        };
      }
    }
  }
...
export const runBayerNativeSamplePreservation = ():
  | { ok: true }
  | { ok: false; reason: string } => {
  ...
  for (const cfa of Object.keys(layouts)) {
    for (const method of ["NEAREST", "BILINEAR", "EDGE_AWARE"]) {
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Bayer ${cfa}/${method} threw: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
      ...
      for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
          ...
          if (Math.abs(output[offset] - input[offset]) > 1) {
            return {
              ok: false,
              reason: `Bayer ${cfa}/${method} changed native sample at ${x},${y},c${channel}: ${input[offset]} -> ${output[offset]}`,
            };
          }
        }
      }
...
export const runBayerTemporalNoise = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const render = (
    ...
    try {
      return canvasPixels(
        filter.func(makeSolidCanvas(64, 48, 112), {
          ...(filter.defaults ?? {}),
          ...runtimeOptions(),
          method: "EDGE_AWARE",
          sensorNoise,
  ...
  for (let index = 0; index < noisyA.length; index += 4) {
    if (
      noisyA[index] !== noisyB[index] ||
      noisyA[index + 1] !== noisyB[index + 1] ||
      noisyA[index + 2] !== noisyB[index + 2]
    )
      noisyChanged += 1;
    if (
      defectA[index] !== defectB[index] ||
      defectA[index + 1] !== defectB[index + 1] ||
      defectA[index + 2] !== defectB[index + 2]
    )
      defectChanged += 1;
  }
  return noisyChanged > 64 * 48 * 0.5 && defectChanged === 0
    ...
    : {
        ok: false,
        reason: `Bayer frame response noisy=${noisyChanged}, fixed-defect=${defectChanged}`,
      };
};
...
export const runBayerGradientCorrection = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `Bayer gradient render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  return correctedRmse < bilinearRmse * 0.8
    ...
    : {
        ok: false,
        reason: `Bayer 5x5 did not improve neutral edge RMSE (${bilinearRmse.toFixed(3)} -> ${correctedRmse.toFixed(3)})`,
      };
};
...
export const runMoireModeSeparation = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (!sensor || !screen || !print)
    return { ok: false, reason: "moiré mode render/readback failed" };
  ...
  return Math.min(sensorScreen, sensorPrint, screenPrint) > 0.35
    ...
    : {
        ok: false,
        reason: `moiré modes insufficiently distinct (${sensorScreen.toFixed(2)}, ${sensorPrint.toFixed(2)}, ${screenPrint.toFixed(2)})`,
      };
};
...
export const runMoireLatticeControls = (): { ok: true } | { ok: false; reason: string } => {
  ...
  return angleChange > 0.35 && pitchChange > 0.35 && zeroDriftChange === 0
    ...
    : {
        ok: false,
        reason: `moiré controls angle=${angleChange.toFixed(2)} pitch=${pitchChange.toFixed(2)} zero-drift=${zeroDriftChange.toFixed(2)}`,
      };
};
...
export const runOscilloscopeSignalModes = (): { ok: true } | { ok: false; reason: string } => {
  ...
  for (const display of ["WAVEFORM", "TRACE", "PARADE"]) {
    ...
    if (meanDelta > 3) {
      return {
        ok: false,
        reason: `oscilloscope ${display} CPU/GL mean delta=${meanDelta.toFixed(2)}`,
      };
    }
    outputs.push(gpu);
  }
  ...
  if (waveformTrace <= 0.2 || waveformParade <= 0.2) {
    return {
      ok: false,
      reason: `oscilloscope modes collapsed (${waveformTrace.toFixed(2)}, ${waveformParade.toFixed(2)})`,
    };
  }
  ...
  if (!binaryGpu || !binaryCpu)
    return { ok: false, reason: "oscilloscope binary-palette readback failed" };
  const channelValues = new Set<number>();
  let maximumDelta = 0;
  for (let index = 0; index < binaryGpu.length; index += 4) {
    for (let channel = 0; channel < 3; channel += 1) {
      ...
      maximumDelta = Math.max(
        maximumDelta,
        Math.abs(binaryGpu[index + channel] - binaryCpu[index + channel]),
      );
    }
  }
  ...
  return binary && maximumDelta <= 1
    ...
    : {
        ok: false,
        reason: `oscilloscope palette mismatch values=${[...channelValues]} maxDelta=${maximumDelta}`,
      };
};
...
export const runCcdChargeAccumulation = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const render = (events: number, antiBlooming: number) => {
    ...
    for (let y = 0; y < 64; y += 1) {
      for (let x = 0; x < 64; x += 1) {
        ...
        const added =
          Math.max(0, output[index] - input[index]) +
  ...
  try {
    ...
    return {
      ok: false,
      reason: `CCD accumulation render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  if (two.energy <= one.energy * 1.45) {
    return {
      ok: false,
      reason: `CCD overload did not accumulate (${one.energy} -> ${two.energy})`,
    };
  }
  ...
  if (drained.energy >= two.energy * 0.5) {
    return {
      ok: false,
      reason: `CCD anti-blooming drain was too weak (${two.energy} -> ${drained.energy})`,
    };
  }
...
export const runLaserSpeckleDiversity = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `laser diversity render threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  return eight.contrast < one.contrast * 0.55 && meanRatio > 0.8 && meanRatio < 1.2
    ...
    : {
        ok: false,
        reason: `laser diversity contrast ${one.contrast.toFixed(3)} -> ${eight.contrast.toFixed(3)}, mean ratio=${meanRatio.toFixed(3)}`,
      };
};
...
export const runCanvasOwnershipReuse = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const renderAndRelease = (): { ok: true } | { ok: false; reason: string } => {
    ...
    if (!lcdOutput || lcdOutput === source)
      return { ok: false, reason: "LCD ownership warm-up failed" };
    ...
    if (!mavicaOutput || mavicaOutput === source)
      return { ok: false, reason: "Mavica ownership warm-up failed" };
  ...
  if (finalCanvas.allocations !== warmCanvas.allocations) {
    return {
      ok: false,
      reason: `canvas allocations did not plateau: ${warmCanvas.allocations} -> ${finalCanvas.allocations}`,
    };
  }
  if (finalGl.readoutCanvases !== warmGl.readoutCanvases) {
    return {
      ok: false,
      reason: `GL readout allocations did not plateau: ${warmGl.readoutCanvases} -> ${finalGl.readoutCanvases}`,
    };
  }
  if (
    finalCanvas.reuses < warmCanvas.reuses + 20 ||
  ...
  try {
    ...
    sawInjectedFailure =
      error instanceof Error && error.message === "injected Mavica pre-JPEG read failure";
  } finally {
