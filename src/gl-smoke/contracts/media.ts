...
export const runPhotocopierToneContinuity = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `copy ramp render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    levels.add(value);
    if (value < previous) return { ok: false, reason: `copy ramp reversed at ${x}: ${previous} -> ${value}` };
    previous = value;
  }
...
export const runPaperTextureHighScaleBounded = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(width, height, 128), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      type: "CANVAS",
      blendMode: "OVERLAY",
    ...
    return { ok: false, reason: `high-scale paper render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  if (!pixels) return { ok: false, reason: "high-scale paper readback failed" };
  let totalDelta = 0, comparisons = 0, low = 255, high = 0;
  ...
  return high - low >= 6 && meanDelta < 38
    ? { ok: true }
    : { ok: false, reason: `high-scale weave range=${high - low}, adjacent delta=${meanDelta.toFixed(2)}` };
};
...
export const runSumiFiberCorrelation = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(width, height, 255), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      washStrength: 0,
      edgeStrength: 0,
    ...
    return { ok: false, reason: `washi correlation render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  if (!pixels) return { ok: false, reason: "washi correlation readback failed" };
  let adjacent = 0, distant = 0, samples = 0;
  for (let y = 0; y < height; y += 1) {
  ...
  return distant > 2 && adjacent < distant * 0.5
    ? { ok: true }
    : { ok: false, reason: `washi was not spatially correlated (adjacent=${adjacent.toFixed(2)}, distant=${distant.toFixed(2)})` };
};
...
export const runRisographNoForcedBleed = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `riso impulse render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  if (!pixels) return { ok: false, reason: "riso impulse readback failed" };
  const centerValue = pixels[(center * size + center) * 4];
  if (centerValue > 20) return { ok: false, reason: `riso impulse center was not printed (${centerValue})` };
...
export const runDuplexPaperHighlights = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(32, 32, 255), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      paperColor: paper,
    }) as HTMLCanvasElement);
  } catch (error) {
    return { ok: false, reason: `duplex highlight render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  if (!pixels) return { ok: false, reason: "duplex highlight readback failed" };
  for (let index = 0; index < pixels.length; index += 4) {
    if (Math.abs(pixels[index] - paper[0]) > 1
      || Math.abs(pixels[index + 1] - paper[1]) > 1
      || Math.abs(pixels[index + 2] - paper[2]) > 1) {
      return { ok: false, reason: `paper highlight was inked rgb=${pixels[index]},${pixels[index + 1]},${pixels[index + 2]}` };
    }
  }
  return { ok: true };
};
...
export const runScreenPrintHalftone = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(96, 72, 128), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      plates: 2,
      offset: 0,
    ...
    return { ok: false, reason: `screen halftone render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  if (!pixels) return { ok: false, reason: "screen halftone readback failed" };
  let paperPixels = 0, inkPixels = 0;
  for (let index = 0; index < pixels.length; index += 4) {
    const distance = Math.abs(pixels[index] - paper[0])
  ...
  return paperPixels > 300 && inkPixels > 300
    ? { ok: true }
    : { ok: false, reason: `screen lacked discrete paper/ink dots (paper=${paperPixels}, ink=${inkPixels})` };
};
...
export const runThermalPrinterCellCoherence = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `thermal-cell render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
        ...
        for (let x = cellX; x < Math.min(width, cellX + cellSize); x += 1) {
          ...
          for (let channel = 0; channel < 3; channel += 1) {
            if (pixels[index + channel] !== pixels[origin + channel]) {
              return { ok: false, reason: `thermal cell ${cellX / cellSize},${cellY / cellSize} contains output-resolution noise` };
            }
          }
        }
      }
...
export const runPolaroidNoForcedBlur = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `Polaroid impulse render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
...
export const runFilmGrainContinuousClusters = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(64, 48, 128), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      amount: 0.7,
      size,
    ...
    return { ok: false, reason: `film-grain render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  ...
  return internallyVaried >= 24
    ? { ok: true }
    : { ok: false, reason: `grain remained square/block-constant (${internallyVaried} varied cells)` };
};
...
export const runLightLeakNeutrality = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(64, 48, 0), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      intensity: 1,
      position: "TL",
    ...
    return { ok: false, reason: `light-leak render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
...
export const runProjectionDustOcclusion = (): { ok: true } | { ok: false; reason: string } => {
  ...
  let pixels: Uint8ClampedArray | null;
  try {
    pixels = canvasPixels(filter.func(makeSolidCanvas(320, 240, 128), {
      ...(filter.defaults ?? {}),
      ...runtimeOptions(),
      gateWeave: 0,
      grain: 0,
    ...
    return { ok: false, reason: `projection-dust render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  ...
  return minimum < 110 && maximum <= 130
    ? { ok: true }
    : { ok: false, reason: `projected dust did not occlude neutral film (range=${minimum}..${maximum})` };
};
...
export const runProjectionScratchDiversity = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `projection-scratch render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
  ...
  return dark > 80 && bright > 80
    ? { ok: true }
    : { ok: false, reason: `scratches lacked base/emulsion diversity (dark=${dark}, bright=${bright})` };
};
...
export const runInfraredNeutrality = (): { ok: true } | { ok: false; reason: string } => {
  ...
  try {
    ...
    return { ok: false, reason: `neutral render threw: ${error instanceof Error ? error.message : String(error)}` };
  }
...
export const runNokiaLcdStates = (): { ok: true } | { ok: false; reason: string } => {
  ...
  const render = (width: number, height: number, pixelGrid: boolean): Uint8ClampedArray | null => {
    try {
      return canvasPixels(filter.func(makeGradientCanvas(width, height), {
        ...(filter.defaults ?? {}),
        ...runtimeOptions(),
        pixelGrid,
      }) as HTMLCanvasElement);
  ...
  for (const [label, pixels] of [["small", small], ["large", largeWithGrid]] as const) {
    const seen = new Set<string>();
    for (let index = 0; index < pixels.length; index += 4) {
      const color = `${pixels[index]},${pixels[index + 1]},${pixels[index + 2]}`;
      if (!legal.has(color)) return { ok: false, reason: `${label} LCD emitted non-physical color ${color}` };
      seen.add(color);
    }
    if (seen.size !== 2) return { ok: false, reason: `${label} LCD did not use both optical states` };
  }
...
export const runThermalNoiseContract = (): { ok: true } | { ok: false; reason: string } => {
  ...
  if (movingChanges === 0) return { ok: false, reason: "enabled temporal noise was frame-invariant" };
  if (stillChanges !== 0) return { ok: false, reason: `disabled temporal noise changed ${stillChanges} channels` };
  return { ok: true };
};
