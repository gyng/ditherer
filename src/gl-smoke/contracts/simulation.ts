...
export const runStableFluidsProjectionStable = (): Result => {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `Stable Fluids threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
...
export const runWakeMotionGated = (): Result => {
  ...
  for (let i = 0; i < moved.length; i += 4) {
    moved[i] = 255 - srcPixels[i];
    moved[i + 1] = 255 - srcPixels[i + 1];
    moved[i + 2] = 255 - srcPixels[i + 2];
    moved[i + 3] = 255;
  }
  ...
  const runWake = (ema: Uint8ClampedArray): Uint8ClampedArray | null => {
    ...
    fresh
      .getContext("2d")!
      .putImageData(new ImageData(new Uint8ClampedArray(srcPixels), w, h), 0, 0);
    return canvasPixels(
      filter.func(fresh, {
        ...(filter.defaults ?? {}),
        intensity: 16,
  ...
  const stillDiff = meanDiff(noMotion),
    motionDiff = meanDiff(motion);
  if (stillDiff > 4) {
...
export const runStableFluidsNoProjectionRenders = (): Result => {
  ...
  try {
    pixels = canvasPixels(
      filter.func(makeGradientCanvas(64, 48), {
        ...(filter.defaults ?? {}),
        ...runtimeOptions(),
    ...
    return {
      ok: false,
      reason: `Stable Fluids (no projection) threw: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  return pixels
    ? { ok: true }
    : { ok: false, reason: "Stable Fluids (no projection) readback failed" };
};
