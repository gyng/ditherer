...
export const runSolarizeReversal = (): Result => {
  ...
  if (!(meanLuma(dark) < 90)) {
    return { ok: false, reason: `shadow tone was pushed up unexpectedly (luma ${meanLuma(dark).toFixed(1)})` };
  }
  if (Math.abs(meanLuma(identity) - 200) > 3) {
    return { ok: false, reason: `strength 0 is not identity (luma ${meanLuma(identity).toFixed(1)})` };
  }
  return { ok: true };
};
...
export const runHazeKoschmiederDepth = (): Result => {
  ...
  const hazed = run("Atmospheric Haze", ramp, { strength: 0.9, depthMode: "LUMA", highlightBloom: 0 });
  const identity = run("Atmospheric Haze", makeSolidCanvas(24, 24, 128),
    { strength: 0, depthMode: "LUMA", highlightBloom: 0 });
  ...
  const blueBiasAt = (fx: number): number => {
    ...
    let sum = 0;
    for (let y = 0; y < h; y += 1) { const i = (y * w + x) * 4; sum += hazed[i + 2] - hazed[i]; }
    return sum / h;
  };
  const near = blueBiasAt(0.25), mid = blueBiasAt(0.5), far = blueBiasAt(0.75);
  if (!(far > mid && mid > near && near > 4)) {
    return { ok: false, reason: `airlight tint not increasing with depth (${near.toFixed(1)}, ${mid.toFixed(1)}, ${far.toFixed(1)})` };
  }
  // Exponential (concave) buildup: near step must exceed the far step. A
  // linear-in-depth transmission would make these equal.
  const stepNear = mid - near, stepFar = far - mid;
  if (!(stepNear > stepFar * 1.25)) {
    return { ok: false, reason: `transmission not exponential (step near ${stepNear.toFixed(1)} vs far ${stepFar.toFixed(1)})` };
  }
...
export const runDodgeBurnDirectional = (): Result => {
  const dodged = run("Dodge / Burn", makeSolidCanvas(20, 20, 60), { mode: "DODGE", strength: 0.6, range: 128 });
  const burned = run("Dodge / Burn", makeSolidCanvas(20, 20, 210), { mode: "BURN", strength: 0.6, range: 128 });
  const identity = run("Dodge / Burn", makeSolidCanvas(20, 20, 150), { mode: "BOTH", strength: 0, range: 128 });
  if (!dodged || !burned || !identity) return { ok: false, reason: "Dodge/Burn readback failed" };

  if (!(meanLuma(dodged) > 60 + 3)) {
    return { ok: false, reason: `dodge did not brighten the shadow (luma ${meanLuma(dodged).toFixed(1)})` };
  }
  if (!(meanLuma(burned) < 210 - 3)) {
    return { ok: false, reason: `burn did not darken the highlight (luma ${meanLuma(burned).toFixed(1)})` };
  }
  ...
  const linMid = run("Dodge / Burn", makeSolidCanvas(16, 16, 128), { mode: "DODGE", strength: 1, range: 255 });
  if (!linMid) return { ok: false, reason: "Dodge/Burn readback failed" };
  if (!(meanLuma(linMid) < 178)) {
    return { ok: false, reason: `exposure applied in gamma space, not linear (luma ${meanLuma(linMid).toFixed(1)})` };
  }
  return { ok: true };
};
