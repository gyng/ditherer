...
export const CHAIN_PRESETS: ChainPreset[] = [
  ...
  { name: "Anime Background", desc: "Daylight environment paint-over with simplified structure, authored value colors, and atmospheric depth", filters: [f("Bilateral blur"), f("Anime Tone Bands", { structureScale: 6, colorSeparation: 0.25, mix: 0.72 }), f("Anime Color Grade", { look: "CLEAR_DAY", mix: 0.82 }), f("Atmospheric Haze", { strength: 0.35 })], category: "Anime & Paint-over" },
  { name: "Anime Clean", desc: "Edge-preserved smoothing, restrained cel values, balanced grading, and clean structural ink", filters: [f("Bilateral blur"), f("Anime Tone Bands", { structureScale: 4, colorSeparation: 0.2, mix: 0.65 }), f("Anime Color Grade", { look: "BALANCED", mix: 0.72 }), f("Anime Ink Lines", { source: "XDOG", threshold: 86, textureSuppression: 0.72, overlayMix: 0.48 })], category: "Anime & Paint-over" },
  {
    ...
    filters: [
      ...
      f("Anime Ink Lines", { source: "XDOG", threshold: 82, sigma: 1.25, textureSuppression: 0.72, lineWidth: 0.9, overlayMix: 0.58 }),
    ],
  ...
  },
  {
    ...
    filters: [
      ...
      f("Anime Tone Bands", { structureScale: 7, highlightSteps: 5, colorSeparation: 0.22, mix: 0.55 }),
      f("Anime Color Grade", { look: "CLEAR_DAY", shadowCool: 0.45, highlightWarm: 0.36, mix: 0.8 }),
  ...
  },
  {
    ...
    filters: [
      ...
      f("Anime Ink Lines", { source: "XDOG", threshold: 104, sigma: 1.4, textureSuppression: 0.78, lineWidth: 0.8, overlayMix: 0.38 }),
    ],
    category: "Anime & Paint-over",
  },
  {
    ...
    filters: [
      f("Anime Tone Bands", { shadowTint: [72, 76, 143], highlightTint: [225, 188, 222], colorSeparation: 0.38, mix: 0.7 }),
      f("Anime Color Grade", { look: "BLUE_HOUR", shadowCool: 0.58, highlightWarm: 0.24, mix: 0.9 }),
  ...
  { name: "Autocorrelation Halo", desc: "Full FFT dephase — image collapses into its own autocorrelation, a symmetric feature halo around the origin", filters: [f("FFT Dephase", { amount: 1 }), f("Bloom")], category: "Advanced" },
  { name: "Ghost Self-Portrait", desc: "Partial dephase plus after-image — half the geometry is preserved, half becomes its own autocorrelation shadow", filters: [f("FFT Dephase", { amount: 0.6 }), f("After-image"), f("Bloom")], category: "Advanced" },
  { name: "Horizontal Strip", desc: "FFT angular wedge isolating horizontal frequencies — keeps scan-line-like structure, removes everything else", filters: [f("FFT Angular Wedge", { angle: 0, wedge: 15 }), f("Bloom")], category: "Advanced" },
  { name: "Weave Killer", desc: "Inverted angular wedge on the diagonal — removes 45° weaves while preserving horizontal/vertical structure", filters: [f("FFT Angular Wedge", { angle: 45, wedge: 20, invert: true })], category: "Advanced" },
  { name: "Phase Skeleton", desc: "FFT phase-only reconstruction — strips magnitude information so only structural edges remain, levels stretch brings them out", filters: [f("FFT Phase Only"), f("Levels")], category: "Advanced" },
  { name: "Illumination Flatten", desc: "Homomorphic filter removes uneven lighting then a sharpen pass brings detail back", filters: [f("FFT Homomorphic"), f("Sharpen")], category: "Advanced" },
  ...
  { name: "Turing Coral", desc: "Gray-Scott coral reaction-diffusion grows out of the image over time", filters: [f("Reaction-Diffusion", { preset: "CORAL" }), f("Bloom")], category: "Advanced" },
  { name: "Turing Spots", desc: "Gray-Scott leopard-spot Turing pattern seeded from the image", filters: [f("Reaction-Diffusion", { preset: "SPOTS", mode: "OVERLAY" })], category: "Advanced" },
  { name: "Turing Mitosis", desc: "Mitosis regime — cell-like bubbles divide and bud continuously", filters: [f("Reaction-Diffusion", { preset: "MITOSIS", mode: "B" }), f("Bloom")], category: "Advanced" },
  { name: "Smoke Trails", desc: "Stam-style fluid advection — image edges inject velocity and smoke streams along them", filters: [f("Stable Fluids", { mode: "OVERLAY" }), f("Bloom")], category: "Advanced" },
  { name: "Velocity Field", desc: "Stable Fluids velocity field visualised as hue-coded flow", filters: [f("Stable Fluids", { mode: "FIELD" })], category: "Advanced" },
  { name: "Caustic Pool", desc: "Treat the image as glass — bright caustic webs where gradients converge", filters: [f("Caustics"), f("Bloom")], category: "Advanced" },
  ...
  { name: "Droste Spiral", desc: "Log-polar recursion à la Escher's Prentententoonstelling — the image wraps into itself", filters: [f("Droste spiral", { twist: 0.35 }), f("Bloom")], category: "Distort" },
  { name: "Möbius Transform", desc: "Complex Möbius warp with subtle chromatic aberration", filters: [f("Möbius transform"), f("Chromatic aberration")], category: "Distort" },
  { name: "Anamorphic Mirror", desc: "Cylindrical anamorphosis — centre looks normal, outer ring stretches logarithmically", filters: [f("Anamorphic Cylinder"), f("Vignette")], category: "Distort" },
  { name: "Fractal Flame (Swirl)", desc: "Swirl IFS variation with layered multi-tap accumulation and bloom", filters: [f("Fractal Flame", { variation: "SWIRL", taps: 5 }), f("Bloom")], category: "Distort" },
  { name: "Fractal Flame (Horseshoe)", desc: "Horseshoe IFS variation with flame-colour tint and glow", filters: [f("Fractal Flame", { variation: "HORSESHOE" }), f("Bloom"), f("Vignette")], category: "Distort" },
  ...
  { name: "Watercolor", desc: "Wet-on-wet pigment diffusion with edge blooms and soft posterization", filters: [f("Watercolor bleed", { iterations: 18, flow: 0.3, wetness: 0.8, edgeBloom: 0.6 }), f("Posterize edges")], category: "Stylize" },
  { name: "Topographic Map", desc: "SDF isolines over a painted fill — feels like a printed contour map", filters: [f("Gaussian blur"), f("SDF Stylize", { mode: "ISOLINES", spacing: 10, thickness: 1 })], category: "Stylize" },
  { name: "Bevelled Emblem", desc: "Distance-field bevel stylisation — the brightest shapes punch out in relief", filters: [f("SDF Stylize", { mode: "BEVEL", spacing: 24 })], category: "Stylize" },
  { name: "Silk Flow", desc: "Line Integral Convolution along edge tangents — silky directional streaks over colour", filters: [f("Line Integral Convolution", { length: 24, steps: 32 })], category: "Stylize" },
  { name: "Ink Flow Study", desc: "Flow-aligned crosshatch ink strokes over the source colour", filters: [f("Flow Crosshatch", { preserveColor: true, spacing: 5 })], category: "Stylize" },
  { name: "Classical Hatching", desc: "Paper-and-ink flow crosshatch with sepia paper", filters: [f("Flow Crosshatch", { spacing: 6 }), f("Sepia")], category: "Stylize" },
  { name: "Wallpaper (Square)", desc: "P4M crystallographic tiling — four-fold square kaleidoscope", filters: [f("Wallpaper Tiling", { group: "P4M", cellSize: 180 })], category: "Stylize" },
  { name: "Wallpaper (Hex)", desc: "P6M hexagonal symmetry tiling — six-fold kaleidoscope", filters: [f("Wallpaper Tiling", { group: "P6M", cellSize: 220 })], category: "Stylize" },
  ...
  { name: "Cyanotype", desc: "Prussian-blue sun-print with paper grain and warm highlight tint — replaces the old Grayscale+Contour+Invert Blueprint recipe", filters: [f("Cyanotype"), f("Film grain")], category: "Simulate" },
  { name: "Blueprint", desc: "Inverted cyanotype — white architectural line drawing on deep blue paper", filters: [f("Line art"), f("Cyanotype", { invert: true, contrast: 1.8 })], category: "Simulate" },
  { name: "Cyberpunk", desc: "Neon-soaked aperture-grille monitor with deliberate chromatic split and bloom glow", filters: [f("Chromatic posterize"), f("Chromatic aberration"), f("Bloom"), f("CRT emulation", { tubeProfile: "APERTURE_GRILLE", strength: 0.38, bloomStrength: 0.35 })], category: "Simulate" },
  { name: "Camera Operator", desc: "Production-monitor false color, focus peaking, zebras, and clipping warnings over a live sensor feed", filters: [f("Bayer Sensor", { sensorNoise: 0.015, hotPixels: 0 }), f("Camera Monitor", { mode: "COMBINED", overlayOpacity: 0.75 })], category: "Simulate" },
  { name: "CMOS Phone Video", desc: "Modern phone-sensor artifacts: demosaic texture, rolling readout skew, and clipped highlight charge", filters: [f("Bayer Sensor", { method: "EDGE_AWARE", sensorNoise: 0.02 }), f("Rolling Shutter", { readout: 0.75, skew: 12, wobble: 1.5 }), f("CCD Charge Smear", { threshold: 0.92, strength: 0.35, length: 7 })], category: "Simulate" },
  { name: "Degauss Ritual", desc: "Consumer-tube wobble and phosphor mislanding followed by an intentionally long afterglow", filters: [f("CRT Degauss"), f("CRT emulation", { tubeProfile: "CONSUMER_525" }), f("Phosphor decay", { profile: "LONG_PERSISTENCE" })], category: "Simulate" },
  { name: "Arcade 240p", desc: "Progressive 240-line arcade raster through a slot mask with current-dependent beam width", filters: [f("CRT emulation", { tubeProfile: "ARCADE_240P", strength: 0.52, beamMinWidth: 0.16, beamMaxWidth: 0.4 })], category: "Simulate" },
  { name: "Aperture Grille Monitor", desc: "Sharper progressive monitor raster with vertical RGB stripes and two stabilizing wires", filters: [f("CRT emulation", { tubeProfile: "APERTURE_GRILLE", phosphorScale: 1, strength: 0.32, curvature: 0.025, overscan: 0.01 })], category: "Simulate" },
  { name: "Broadcast CRT", desc: "Controlled studio monitor geometry with broadcast overscan, restrained convergence error, and P22 timing", filters: [f("CRT emulation", { tubeProfile: "BROADCAST", strength: 0.3, misconvergence: 0.1, bloomStrength: 0.14 }), f("Phosphor decay", { profile: "P22_COLOR", refreshRate: 60 })], category: "Simulate" },
  ...
  { name: "Glass Sculpture", desc: "Image-derived glass relief with prismatic edges and a soft gallery glow", filters: [f("Refractive Glass", { surface: "LUMINANCE", refraction: 18, dispersion: 0.5, highlight: 0.35 }), f("Chromatic aberration", { amount: 2 }), f("Bloom")], category: "Simulate" },
  { name: "Lenticular Card", desc: "Holographic rainbow sheen photographed through a fine aliasing grid with scanlines and glow", filters: [f("Lenticular"), f("Moiré / Aliasing", { pattern: "SCREEN", cellSize: 3, strength: 0.22, chroma: 0.45 }), f("Scanline"), f("Bloom")], category: "Simulate" },
  { name: "Lo-fi Webcam", desc: "Cheap CMOS readout, chunky pixels, JPEG artifacts, and noisy optics", filters: [f("Bayer Sensor", { method: "BILINEAR", sensorNoise: 0.05, hotPixels: 0.002 }), f("Rolling Shutter", { readout: 0.65, skew: 8, wobble: 2 }), f("Pixelate"), f("JPEG artifact"), f("Film grain"), f("Vignette")], category: "Simulate" },
  { name: "Metadata Mistake", desc: "Wrong decode assumptions layered over wavelet and block-codec damage with hard edge recovery", filters: [f("Metadata Mismatch Decode"), f("Wavelet Codec", { scale: 4, quality: 28, detailLoss: 0.8 }), f("JPEG artifact"), f("Sharpen")], category: "Simulate" },
  { name: "Mavica Photo", desc: "Sony Mavica floppy-disk camera with CCD highlight smear, heavy JPEG, and sensor grain", filters: [f("CCD Charge Smear", { threshold: 0.86, strength: 0.55, length: 10, decay: 0.8 }), f("Mavica FD7"), f("JPEG artifact"), f("Film grain")], category: "Simulate" },
  ...
  { name: "Retro TV", desc: "NTSC tape playback through a 525/60 consumer shadow-mask tube with rounded-screen falloff", filters: [f("VHS / NTSC"), f("CRT emulation", { tubeProfile: "CONSUMER_525", strength: 0.42, overscan: 0.04 }), f("Vignette")], category: "Simulate" },
  { name: "Screen Photograph", desc: "A camera pointed at a display: Bayer reconstruction, rotated RGB moiré, lens warp, and dark corners", filters: [f("Moiré / Aliasing", { pattern: "SCREEN", cellSize: 4, angle: 8, strength: 0.7, chroma: 0.65 }), f("Bayer Sensor", { method: "BILINEAR", colorBleed: 0.5 }), f("Lens distortion", { k1: 0.12 }), f("Vignette")], category: "Simulate" },
  { name: "Security Camera", desc: "Monochrome rolling-shutter feed with an explicit motion-analysis overlay for moving subjects", filters: [f("Rolling Shutter", { readout: 0.7, skew: 6, wobble: 1 }), f("Grayscale"), f("Motion Analysis", { renderMode: "MASK", source: "EMA" }), f("Scanline"), f("Film grain")], category: "Simulate" },
  ...
  { name: "Underwater", desc: "Motion-reactive water turbulence refracted through uneven glass with chromatic split", filters: [f("Wake turbulence"), f("Refractive Glass", { surface: "FROSTED", refraction: 7, roughness: 0.28, dispersion: 0.18 }), f("Chromatic aberration"), f("Bloom")], category: "Simulate" },
  { name: "Wavelet Breakdown", desc: "Low-quality wavelet reconstruction pushed through metadata damage and sharp digital recovery", filters: [f("Wavelet Codec", { scale: 8, channels: "CHROMA", quality: 16, detailLoss: 0.9, ringing: 0.5 }), f("Metadata Mismatch Decode"), f("Sharpen")], category: "Simulate" },
  {
    name: "Vinyl Record",
