...
import type {
  SourceVideoWithObjectUrl,
  VideoFrameCallbackVideo,
  VideoFrameMetadata,
} from "../helpers";
...
export const useSaveAsRenderSync = ({
  ...
  const estimateVideoFps = useCallback((vid: HTMLVideoElement, fallback: number) => {
    ...
    if (qualityFrames && duration > 0)
      return Math.max(1, Math.min(60, Math.round(qualityFrames / duration)));
    if (anyVid.webkitDecodedFrameCount && duration > 0)
      return Math.max(1, Math.min(60, Math.round(anyVid.webkitDecodedFrameCount / duration)));
    if (anyVid.mozPresentedFrames && duration > 0)
      return Math.max(1, Math.min(60, Math.round(anyVid.mozPresentedFrames / duration)));
    return fallback;
  }, []);

  const waitForRenderedSeek = useCallback(
    async (
      ...
      const settleCount = strictValidation
        ? Math.max(1, settleFrames)
        : Math.max(1, Math.min(2, settleFrames));
      ...
      if (strictValidation && "requestVideoFrameCallback" in vid) {
        await Promise.race([
          new Promise<void>((resolve) => {
            const callbackId = (vid as VideoFrameCallbackVideo).requestVideoFrameCallback?.(
            ...
            window.setTimeout(
              () => {
                if (
                  typeof (vid as VideoFrameCallbackVideo).cancelVideoFrameCallback === "function" &&
                  callbackId != null
                ) {
                  try {
      ...
      while (performance.now() < deadline) {
        ...
        const decodedMatches =
          decodedMediaTime == null || Math.abs(decodedMediaTime - targetTime) <= targetTolerance;
        const videoMatches = Math.abs((vid.currentTime || 0) - targetTime) <= targetTolerance;
        const inputTimeMatches =
          latestState.time != null && Math.abs(latestState.time - targetTime) <= targetTolerance;
        const outputTimeMatches =
          latestState.outputTime != null &&
          Math.abs(latestState.outputTime - targetTime) <= targetTolerance;
        ...
        const outputTokenCaughtUp =
          outputFrameToken === inputFrameToken && outputFrameToken > previousInputFrameToken;
        const hasOutput = !!latestState.outputImage;
        const renderCaughtUp =
          outputTokenCaughtUp || (renderVersionRef.current > previousRenderVersion && hasOutput);
        if (
          decodedMatches &&
          videoMatches &&
          inputTimeMatches &&
  ...
  const waitForRenderedPlaybackFrame = useCallback(
    async (
      targetTime: number,
      ...
      while (performance.now() < deadline) {
        ...
        const renderAdvanced =
          renderVersionRef.current > previousRenderVersion ||
          outputFrameToken > previousInputFrameToken;
        const hasOutput = !!latestState.outputImage;
        const inputTimeMatches =
          stateTime != null && Math.abs(stateTime - targetTime) <= targetTolerance;
        const outputTimeMatches =
          outputTime != null && Math.abs(outputTime - targetTime) <= targetTolerance;
  ...
  const waitForVideoSeekSettled = useCallback(
    async (
      vid: HTMLVideoElement,
      targetTime: number,
      expectedFrameMs = 1000 / Math.max(1, gifFps),
