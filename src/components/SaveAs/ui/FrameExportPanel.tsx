  ...
  return (
    <>
      <div className={s.row}>
        <span className={s.rowLabel}>
          ...
          <span
            ...
            title="Choose between realtime playback, Offline Render (Browser), or Offline Render (WebCodecs)."
          >
            (i)
          </span>
        </span>
        <Enum
          ...
          onSetFilterOption={(_, value) =>
            onSetLoopCaptureMode(value as "offline" | "realtime" | "webcodecs")
          }
        />
      </div>
        ...
        <div className={s.row}>
          <span className={s.rowLabel}>
            ...
            <span
              ...
              title={
                videoFormat === "contact"
                  ? "Number of frames to sample into the contact sheet."
                  : "Number of frames to export when rendering from the current live output instead of a source video loop."
              }
            >
              (i)
            </span>
      ...
      {videoFormat === "contact" && (
        <div className={s.row}>
          <span className={s.rowLabel}>
            ...
            <span
              className={s.inlineInfo}
              title="How many thumbnails to place in each row of the contact sheet."
            >
              (i)
            </span>
          </span>
            ...
            <span
              ...
              title={
                videoFormat === "gif"
                  ...
                  : videoFormat === "sequence"
                    ? "Choose whether sequence export covers the whole video or only a selected timestamp range."
                    : "Choose whether the contact sheet samples the whole video or only a selected timestamp range."
              }
            >
              (i)
            </span>
          ...
          <div className={s.row}>
            <span className={s.rowLabel}>
              ...
              <span
                ...
                title={
                  videoFormat === "contact"
                    ? "Start timestamp for contact sheet sampling."
                    : "Start timestamp for GIF or sequence export."
                }
              >
                (i)
              </span>
            ...
            <div className={s.sliderRow}>
              <input
                ...
                onChange={(event) =>
                  onSetLoopRangeStart(
                    Math.min(parseFloat(event.target.value) || 0, Math.max(0, loopRangeEnd - 0.01)),
                  )
                }
              />
          ...
          <div className={s.row}>
            <span className={s.rowLabel}>
              ...
              <span
                ...
                title={
                  videoFormat === "contact"
                    ? "End timestamp for contact sheet sampling."
                    : "End timestamp for GIF or sequence export."
                }
              >
                (i)
              </span>
                ...
                onChange={(event) =>
                  onSetLoopRangeEnd(
                    Math.max(
                      ...
                      Math.min(videoDuration, loopRangeStart + 0.01),
                    ),
                  )
                }
              />
      ...
      {videoFormat === "gif" && gifUrl && (
        <>
          <img src={gifUrl} className={s.videoPreview} alt="GIF export preview" />
          {gifResultLabel && <div className={s.helperText}>{gifResultLabel}</div>}
        </>
      )}
      ...
      {videoFormat === "sequence" && sequenceBlob && (
        <div className={s.helperText}>Sequence ZIP ready to save or copy.</div>
      )}
      ...
      {videoFormat === "contact" && contactSheetUrl && (
        <img src={contactSheetUrl} className={s.videoPreview} alt="Contact sheet export preview" />
      )}

      {videoFormat === "contact" && contactSheetBlob && (
        <div className={s.helperText}>Contact sheet PNG ready to save or copy.</div>
      )}
