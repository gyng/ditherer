...
**Fix:** Compute `levels` _after_ the `thresholdMaps[thresholdMap]` lookup, from `threshold.levels` or the resolved matrix dimensions.
...
```ts
const diff =
  (abs(buf[i] - ema[i]) + abs(buf[i + 1] - ema[i + 1]) + abs(buf[i + 2] - ema[i + 2])) / 3;
...
```ts
const motion =
  (abs(buf[i] - ema[i]) + abs(buf[i + 1] - ema[i + 1]) + abs(buf[i + 2] - ema[i + 2])) / 3 / 255;
...
When a bright object moves away, it leaves a complementary-colored ghost — like staring at a light then looking away. Simulates retinal fatigue / cone adaptation. Distinct from Phosphor Decay (which keeps the _same_ color persisting) — this inverts the lingering ghost.
...
```ts
const motion = ema
  ? (abs(buf[i] - ema[i]) + abs(buf[i + 1] - ema[i + 1]) + abs(buf[i + 2] - ema[i + 2])) / 765
  : 0;
...
Current datamosh displaces blocks randomly when motion is detected. Real datamosh propagates motion vectors from P-frames — blocks slide _in the direction of motion_, not randomly. Use frame differencing from `_prevInput` to estimate motion direction per block.
