...
Pekka Väänänen, _Revisiting Yliluoma's ordered dither algorithm_
...
All are _N-candidate_ ordered dithers: for each pixel independently, collect `N`
...
_and_ palette (libimagequant's `(0.5, 1, 0.45)` weights at exponent `0.57/0.45455`)
...
| K=16    | 100 ms                           | 83 ms                           |
| K=64    | 328 ms                           | 346 ms                          |
| K=256   | 343 ms _(26 colors — truncated)_ | 949 ms _(163 colors — correct)_ |

Raising the cap costs small palettes nothing (the inner loops early-exit on
`u_paletteCount`, so cost tracks _actual_ K), it compiles, and K=256 renders
...
- On uniforms, `MAX_PAL=256` is no worse than what already ships: it needs 512
  ...
  only _new_ risk is the weights array.
...
2. **Wall-clock timing measures the readback, not the shader.** `readoutToCanvas`
   ...
   contradicted themselves — K=64 timing _faster_ than K=16 — because the signal
...
3. **Timer queries need the pipeline drained.** Without a `gl.finish()` on both
   ...
   cost, others at a floor that was _identical across different configurations_,
...
also violates physics outright — at cap=128, K=64 measures _cheaper_ than K=8;
at cap=256 cost _falls_ as K rises — and half the cells still spread 5×.
...
if it reproduces across runs _and_ rises with K.
...
`colorspace` ENUM applies to distance math _and_ luma ordering, matching the
