...
- **WebGL2 benchmarks default to swiftshader (software renderer)** — GL numbers are slower than real GPU hardware. When benchmarking GL vs WASM, use `_webglAcceleration: false` to isolate the WASM path. The GL fast path is still correct and will outperform WASM on a real GPU; swiftshader results do not indicate a regression for end users.
- **A real GPU is reachable** — `PLAYWRIGHT_GPU=1 PLAYWRIGHT_ANGLE=gl` routes Playwright through Mesa's d3d12 driver to the actual adapter, headless, with nothing needed in your shell. Required for any question a software rasterizer can't answer (register pressure, occupancy, "fast enough for video"), because swiftshader and llvmpipe model none of it. **Print `UNMASKED_RENDERER_WEBGL` and read it before trusting a GL number** — limits differ enough to mislead you (`MAX_FRAGMENT_UNIFORM_VECTORS` is 4096 on swiftshader, 1024 on an RTX 3080). Wall-clock GL timing measures the readback, not the shader; use `EXT_disjoint_timer_query_webgl2`, report spread, and reproduce before believing a result. See [docs/gpu-benchmarking.md](docs/gpu-benchmarking.md) for the method, the traps, and a worked example of getting it wrong.
