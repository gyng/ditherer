...
export const nightVisionIntensifierResponse = (luminance: number, gain: number): number => {
...
export const nightVisionNoiseAmplitude = (amplifiedSignal: number, grain: number): number => {
...
export const ultrasoundBackscatter = (
  ...
  const localMismatch = Math.abs(
    c -
      (clamp01(axialBefore) +
        clamp01(axialAfter) +
        clamp01(lateralBefore) +
        clamp01(lateralAfter)) *
        0.25,
  );
...
export const ultrasoundRayleighEnvelope = (uniform: number, mix = 1): number => {
