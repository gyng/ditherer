...
const motionBlurFilter = (input: any, options = defaults) => {
  ...
  // Physical line integral of light along the motion path: each bilinear
  // sample is a raw sRGB tap (matching the GL hardware-filtered texel), which
  // is then linearized before accumulating so bright/dark taps average as
  // radiant energy, not gamma-coded values. Alpha is not blurred — the
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      for (let t = -halfLen; t <= halfLen; t++) {
        ...
        slg += srgbToLinear(g / 255);
        slb += srgbToLinear(b / 255);
        count++;
      }

      const di = getBufferIndex(x, y, W);
      const r = Math.round(linearToSrgb(slr / count) * 255);
      const g = Math.round(linearToSrgb(slg / count) * 255);
      const b = Math.round(linearToSrgb(slb / count) * 255);
      const a = buf[di + 3];
