...
export const optionTypes = {
  ...
  threshold: {
    ...
    step: 0.01,
    default: 0.5,
    desc: "Luminance threshold for the binary mask",
  },
  spacing: {
    type: RANGE,
    range: [2, 80],
    step: 1,
    default: 16,
    desc: "Isoline or band spacing in pixels",
  },
  thickness: {
    type: RANGE,
    range: [0.5, 10],
    step: 0.5,
    default: 1.5,
    desc: "Isoline thickness in pixels",
  },
      ...
      if (field) {
        ...
        drawPass(
          ...
          () => {
            ...
            gl.uniform3f(
              program.uniforms.u_lineColor,
              options.lineColor[0],
              options.lineColor[1],
              options.lineColor[2],
            );
            gl.uniform3f(
              program.uniforms.u_fillColor,
              options.fillColor[0],
              options.fillColor[1],
              options.fillColor[2],
            );
        ...
        if (rendered) {
          ...
          if (output) {
            logFilterBackend(
              "SDF Stylize",
              "WebGL2",
              `${options.mode} spacing=${options.spacing}${identity ? "" : "+palettePass"}`,
            );
            return output;
          }
        }
