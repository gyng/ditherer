...
export const optionTypes = {
  ...
  angle: {
    ...
    step: 5,
    default: 35,
    desc: "Gradient direction in degrees",
  },
  style: {
    type: ENUM,
    options: [
  ...
  amount: {
    ...
    step: 0.05,
    default: 0.85,
    desc: "How strongly the ordered dither pattern perturbs the gradient before quantization",
  },
  sourceInfluence: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.75,
    desc: "How much the source luminance remaps the generated gradient",
  },
  detailInfluence: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.55,
    desc: "How much source edges intensify local dither contrast",
  },
  sourceColorMix: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.55,
    desc: "How much source color tints the gradient result",
    visibleWhen: (options: any) => options.style === STYLE.DREAMY,
  },
      ...
      const warpedT =
        style === STYLE.PRINT
          ...
          : Math.max(
              ...
              Math.min(
                1,
                baseT * (1 - sourceInfluence * 0.65) +
                  srcLuma * sourceInfluence * 0.65 +
                  edge * 0.12,
              ),
            );
...
export default defineFilter({
  ...
  defaults,
  description:
    "Map a gradient through the source image's luminance and edge structure, then ordered-dither the result",
});
