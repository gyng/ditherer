...
export const optionTypes = {
  resolution: {
    ...
    step: 10,
    default: 200,
    desc: "Print resolution in pixels wide",
  },
  fadeGradient: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.3,
    desc: "Thermal fade toward paper edges",
  },
  dotDensity: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.8,
    desc: "Heat/density response of each fixed print-head dot",
  },
  palette: {
    type: PALETTE,
    default: nearest,
    desc: "Optional output palette; defaults to binary receipt-paper tones",
  },
};
...
const thermalPrinter = (input: any, options: ThermalPrinterOptions = defaults) => {
  ...
  logFilterBackend(
    "Thermal Printer",
    "WebGL2",
    `res=${resolution} density=${dotDensity}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Static direct-thermal receipt print on a discrete line-head dot lattice with density response and edge fading",
  requiresGL: true,
});
