...
export const optionTypes = {
  ...
  threshold: {
    ...
    step: 1,
    default: 20,
    desc: "Pixel difference needed to classify a region as moving",
  },
  feather: {
    type: RANGE,
    range: [0, 20],
    step: 1,
    default: 5,
    desc: "Soft edge around the motion mask",
  },
  background: {
    type: ENUM,
    options: [
  ...
  bgColor: {
    ...
    visibleWhen: (options: BackgroundSubtractionOptions) =>
      options.mode === MODE.FOREGROUND && options.background === BACKGROUND.SOLID,
  },
  learnRate: {
    type: RANGE,
    range: [0.001, 0.1],
...
const sceneSeparation = (input: any, options: BackgroundSubtractionOptions = defaults) => {
  ...
  if (
    mode === MODE.FREEZE_STILL &&
    frozenMode === FROZEN.FIRST &&
