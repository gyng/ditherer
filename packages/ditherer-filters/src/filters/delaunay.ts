...
export const optionTypes = {
  pointCount: {
    type: RANGE,
    range: [50, POINT_COUNT_CAP],
    step: 10,
    default: 300,
    desc: "Number of triangulation vertices",
  },
  edgeWeight: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.5,
    desc: "Bias points toward image edges vs random",
  },
  ...
  seed: {
    ...
    step: 1,
    default: 42,
    desc: "Random seed for point placement",
  },
...
const triangulate = (points: { x: number; y: number }[], W: number, H: number) => {
  ...
  const circumscribes = (tri: Tri, px: number, py: number) => {
    const [ai, bi, ci] = tri;
    const ax = allPts[ai].x,
      ay = allPts[ai].y;
    const bx = allPts[bi].x,
      by = allPts[bi].y;
    const cx = allPts[ci].x,
      cy = allPts[ci].y;
...
export default defineFilter({
  ...
  defaults,
  noWASM:
    "Bowyer-Watson triangulation is an incremental pointer-chasing algorithm with per-insertion work dominated by bad-triangle searches — no parallelism to unlock.",
