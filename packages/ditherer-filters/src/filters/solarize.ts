...
export const optionTypes = {
  threshold: { type: RANGE, range: [0, 255], step: 1, default: 128, desc: "Reversal point — the tone at which the Sabattier curve turns over" },
  strength: { type: RANGE, range: [0, 1], step: 0.05, default: 1, desc: "Reversal strength (0 = no change, 1 = full tone reversal)" },
...
const solarize = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  logFilterBackend("Solarize", "WebGL2", `reversal=${threshold} strength=${strength}${identity ? "" : "+palettePass"}`);
  return out ?? input;
};
