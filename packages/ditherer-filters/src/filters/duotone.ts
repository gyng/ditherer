...
import {
  normalizeColorOption, normalizePaletteOption, normalizeRangeOption,
...
export const optionTypes = {
  shadowColor: { type: COLOR, default: [13, 2, 33], desc: "Shadow ink laid over paper through the deep tones" },
  highlightColor: { type: COLOR, default: [255, 107, 107], desc: "Second ink overprinting through the midtones" },
  paperColor: { type: COLOR, default: [244, 237, 224], desc: "Paper stock shown through the highlights" },
  shadowCurve: { type: RANGE, range: [0.5, 2], step: 0.05, default: 1, desc: "Gamma of the shadow-ink density curve" },
  highlightCurve: { type: RANGE, range: [0.5, 2], step: 0.05, default: 1, desc: "Width of the second-ink midtone density bump" },
...
const duotone = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const rendered = renderDuotoneGL(input, W, H,
      [shadow[0], shadow[1], shadow[2]],
      [highlight[0], highlight[1], highlight[2]],
      [paper[0], paper[1], paper[2]],
      shadowCurve, highlightCurve,);
...
export default defineFilter({
  ...
  optionTypes,
  defaults,
  description: "Two-ink print duotone: shadow and midtone inks with separate density curves overprinted over paper for a true tonal crossover",
  requiresGL: true });
