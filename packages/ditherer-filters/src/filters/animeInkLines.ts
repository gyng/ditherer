...
import {
  clamp, cloneCanvas, fillBufferPixel, getBufferIndex, logFilterBackend,
  logFilterWasmStatus, rgba, srgbPaletteGetColor,
} from "../utils/index";
...
const laplacianEdges = (lum: Float32Array, W: number, H: number) => {
  ...
  for (let y = 1; y < H - 1; y += 1) {
    for (let x = 1; x < W - 1; x += 1) {
      const index = y * W + x;
      out[index] = Math.abs(lum[index - 1]! + lum[index + 1]!
        + lum[index - W]! + lum[index + W]! - lum[index]! * 4);
    }
  }
  return out;
};
...
export const optionTypes = {
  ...
  threshold: { type: RANGE, range: [5, 180], step: 1, default: 74, desc: "Minimum structural edge strength that becomes an ink line" },
  sigma: { type: RANGE, range: [0.5, 4], step: 0.1, default: 1.15, desc: "Fine Gaussian scale used by XDoG line extraction" },
  scaleRatio: { type: RANGE, range: [1.2, 4], step: 0.1, default: 2.2, desc: "Ratio between XDoG fine and coarse Gaussian scales" },
  lineSoftness: { type: RANGE, range: [4, 80], step: 1, default: 34, desc: "Soft-threshold slope of XDoG ink edges" },
  textureSuppression: { type: RANGE, range: [0, 1], step: 0.05, default: 0.62, desc: "Reject weak scale-space responses caused by photographic texture" },
  lineWidth: { type: RANGE, range: [0.1, 4], step: 0.1, default: 1, desc: "Thickness of the final ink contour" },
  ...
  overlayMix: { type: RANGE, range: [0, 1], step: 0.05, default: 0.72, desc: "Opacity of ink over the source in Overlay mode" },
...
const getCache = (gl: WebGL2RenderingContext): Cache => {
  ...
  cache = {
    ...
    render: linkProgram(gl, RENDER_FS, [
      "u_source", "u_fine", "u_coarse", "u_res", "u_mode", "u_threshold",
      "u_softness", "u_textureSuppression", "u_radius", "u_reach",
      "u_lineColor", "u_bgColor", "u_overlay", "u_overlayMix",
    ] as const),
  };
  return cache;
};
...
const runBlur = (
  ...
  const pass = (source: TexEntry, target: TexEntry, x: number, y: number) => {
    drawPass(gl, target, W, H, program, () => {
...
const renderGL = (input: HTMLCanvasElement | OffscreenCanvas, options: typeof defaults) => {
  ...
  const programs = getCache(gl), vao = getQuadVAO(gl);
  ...
  drawPass(gl, luma, W, H, programs.luma, () => {
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, source.tex);
  ...
  drawPass(gl, null, W, H, programs.render, () => {
    gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, source.tex);
    gl.uniform1i(programs.render.uniforms.u_source, 0);
    gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, fine.tex);
    gl.uniform1i(programs.render.uniforms.u_fine, 1);
    gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, coarse.tex);
    ...
    gl.uniform3f(programs.render.uniforms.u_lineColor, options.lineColor[0]! / 255, options.lineColor[1]! / 255, options.lineColor[2]! / 255);
    gl.uniform3f(programs.render.uniforms.u_bgColor, options.bgColor[0]! / 255, options.bgColor[1]! / 255, options.bgColor[2]! / 255);
    gl.uniform1i(programs.render.uniforms.u_overlay, options.renderMode === RENDER_MODE.OVERLAY ? 1 : 0);
...
const animeInkLines = (input: any, options: Record<string, any> = defaults) => {
  ...
  if (glAvailable() && (options as { _webglAcceleration?: boolean })._webglAcceleration !== false) {
    ...
    if (rendered) {
      ...
      if (out) {
        logFilterBackend("Anime Ink Lines", "WebGL2", `${resolved.source}${identity ? "" : "+palettePass"}`);
        return out;
      }
    }
  }
  ...
  const inputContext = input.getContext("2d"), outputContext = output.getContext("2d");
  ...
  const magnitude = String(resolved.source) === EDGE_SOURCE.LAPLACIAN
  ...
  for (let y = 0; y < H; y += 1) {
    for (let x = 0; x < W; x += 1) {
      const index = getBufferIndex(x, y, W), edge = edges[y * W + x] === 1;
      const base = overlay ? [source[index]!, source[index + 1]!, source[index + 2]!] : resolved.bgColor;
      const amount = edge ? (overlay ? clamp(0, 1, resolved.overlayMix) : 1) : 0;
      const color = srgbPaletteGetColor(resolved.palette, rgba(
        Math.round(base[0]! + (resolved.lineColor[0]! - base[0]!) * amount),
        Math.round(base[1]! + (resolved.lineColor[1]! - base[1]!) * amount),
        Math.round(base[2]! + (resolved.lineColor[2]! - base[2]!) * amount), 255,
