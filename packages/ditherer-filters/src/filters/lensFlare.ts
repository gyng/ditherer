...
export const optionTypes = {
  ...
  bloomRadius: { type: RANGE, range: [0.03, 0.35], step: 0.01, default: 0.14, desc: "Bloom radius as a fraction of the image's short side" },
  ...
  ghostSpread: { type: RANGE, range: [0, 1.5], step: 0.05, default: 1, desc: "How far ghost reflections span along the optical axis" },
  streakStrength: { type: RANGE, range: [0, 1], step: 0.05, default: 0.35, desc: "Brightness of the horizontal anamorphic streak" },
  chromaticSpread: { type: RANGE, range: [0, 1], step: 0.05, default: 0.25, desc: "Color separation around ghost reflection edges" },
...
const finiteOr = (value: number, fallback = 0): number => Number.isFinite(value) ? value : fallback;
...
export const lensFlareGhostPosition = (
  ...
  return [
    cx + (cx - sx) * axisPosition * safeSpread,
    cy + (cy - sy) * axisPosition * safeSpread,
  ];
};
...
const lensFlare = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const rendered = renderLensFlareGL(input, W, H,
      ...
      ghosts, bloomRadius, ghostSpread, streakStrength, chromaticSpread,
    );
