...
export const optionTypes = {
  ...
  compositeSharpness: {
    ...
    step: 0.05,
    default: 1,
    desc: "Pre-emphasis applied before composite modulation",
  },
  compositeNoise: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.05,
    desc: "Noise injected into the modulated composite waveform",
  },
  snow: {
    type: RANGE,
    range: [0, 0.1],
    step: 0.00025,
    default: 0.00025,
    desc: "Sparse RF transients and white speckles",
  },
  snowAnisotropy: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.5,
    desc: "Clump snow into horizontal tape lines",
  },
  headSwitching: {
    type: RANGE,
    range: [0, 160],
    step: 1,
    default: 72,
    desc: "Horizontal displacement in the bottom head-switching band",
  },
  headSwitchingHeight: {
    type: RANGE,
    range: [0, 64],
    step: 1,
    default: 8,
    desc: "Height of the bottom head-switching band",
  },
  trackingNoise: {
    type: RANGE,
    range: [0, 60],
    step: 1,
    default: 15,
    desc: "Wavy time-base error near the bottom of the image",
  },
  trackingHeight: {
    type: RANGE,
    range: [0, 96],
    step: 1,
    default: 12,
    desc: "Height of the tracking disturbance region",
  },
  edgeWave: {
    type: RANGE,
    range: [0, 8],
    step: 0.1,
    default: 0.5,
    desc: "Slow whole-frame VHS edge waviness",
  },
  lumaSmear: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.5,
    desc: "Asymmetric horizontal luminance smear",
  },
  tapeSharpness: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.25,
    desc: "ntsc-rs VHS luma sharpen stage, applied after tape filtering and chroma loss",
  },
  ringing: {
    type: RANGE,
    range: [0, 8],
    step: 0.1,
    default: 4,
    desc: "ntsc-rs ringing intensity applied through a calibrated notch filter",
  },
  ringingFrequency: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.45,
    desc: "Normalized frequency of the ntsc-rs luma ringing notch",
  },
  ringingPower: {
    type: RANGE,
    range: [0.25, 8],
    step: 0.25,
    default: 4,
    desc: "Quality factor (power) of the ntsc-rs luma ringing notch",
  },
  lumaNoise: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.01,
    desc: "Noise added after luma demodulation",
  },
  chromaNoise: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.1,
    desc: "Low-frequency noise in the decoded color signal",
  },
  chromaPhaseNoise: {
    type: RANGE,
    range: [0, 1],
    step: 0.001,
    default: 0.001,
    desc: "Random per-line hue flutter from chroma carrier timing error",
  },
  chromaPhaseError: {
    type: RANGE,
    range: [-1, 1],
    step: 0.01,
    default: 0,
    desc: "Constant chroma phase error, expressed as a half-turn",
  },
  chromaDelayH: {
    type: RANGE,
    range: [-12, 12],
    step: 0.5,
    default: 0,
    desc: "Additional horizontal chroma delay in pixels",
  },
  chromaDelayV: {
    type: RANGE,
    range: [-4, 4],
    step: 1,
    default: 0,
    desc: "Additional vertical chroma delay in scanlines",
  },
  chromaLoss: {
    type: RANGE,
    range: [0, 0.25],
    step: 0.000025,
    default: 0.000025,
    desc: "Probability that a scanline loses all color",
  },
  chromaVertBlend: {
    type: BOOL,
    default: true,
    desc: "Blend each chroma scanline with the preceding line as VHS does",
  },
  randomSeed: {
    type: RANGE,
    range: [0, 9999],
    step: 1,
    default: 0,
    desc: "Deterministic noise and defect seed",
  },
  animSpeed: {
    type: RANGE,
    range: [1, 30],
...
const TAPE_PROFILE: Record<
  string,
  { id: number; lumaRadius: number; chromaRadius: number; chromaDelay: number }
> = {
