...
export const optionTypes = {
  ...
  contrast: {
    ...
    step: 0.1,
    default: 0,
    desc: "Video-drive gain around reference mid-gray; 20 units doubles the gain",
  },
  strength: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.45,
    desc: "Visibility of the phosphor mask after mean-light compensation",
  },
  brightness: {
    type: RANGE,
    range: [-64, 64],
    step: 1,
    default: 0,
    desc: "Black-level offset applied to tube drive voltage",
  },
  exposure: {
    type: RANGE,
    range: [0, 4],
    step: 0.05,
    default: 1,
    desc: "Electron-beam current multiplier in linear light",
  },
  gamma: {
    type: RANGE,
    range: [1.6, 3],
    step: 0.05,
    default: 2.4,
    desc: "CRT electro-optical power exponent; 2.4 matches the reference-display model",
  },
  phosphorScale: {
    type: RANGE,
    range: [1, 6],
  ...
  visibleScanlines: {
    ...
    step: 1,
    default: 240,
    desc: "Active raster lines in Custom mode, independent of output resolution",
  ...
  scanlineGap: {
    ...
    step: 1,
    default: 3,
    desc: "Legacy pixel-row spacing retained for saved-chain compatibility",
  },
  scanlineStrength: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.72,
    desc: "Depth of the dark gap between Gaussian raster beam centers",
  },
  beamMinWidth: {
    type: RANGE,
    range: [0.08, 0.7],
    step: 0.01,
    default: 0.18,
    desc: "Gaussian raster spot width at low beam current, in line-pitch units",
  },
  beamMaxWidth: {
    type: RANGE,
    range: [0.12, 0.9],
    step: 0.01,
    default: 0.42,
    desc: "Wider raster spot at high beam current from electron-beam blooming",
  },
  cornerFocus: {
    type: RANGE,
    range: [0, 0.4],
    step: 0.01,
    default: 0.1,
    desc: "Additional beam widening toward the deflection extremes",
  },
  shadowMask: {
    type: ENUM,
    options: [
  ...
  misconvergence: {
    ...
    step: 0.25,
    default: 0.25,
    desc: "Residual RGB convergence error that grows toward screen edges",
  },
  beamSpread: {
    type: RANGE,
    range: [0, 8],
    step: 1,
    default: 0,
    desc: "Horizontal video-bandwidth softness after raster formation",
  },
  ...
  bloomThreshold: {
    ...
    step: 1,
    default: 190,
    desc: "Linear-light beam level where faceplate halo begins",
  },
  ...
  bloomStrength: {
    ...
    step: 0.05,
    default: 0.22,
    desc: "Faceplate halo intensity above the bloom threshold",
  },
  curvature: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.08,
    desc: "Glass and deflection barrel curvature",
  },
  overscan: {
    type: RANGE,
    range: [0, 0.12],
    step: 0.005,
    default: 0.025,
    desc: "Picture cropped beyond the visible raster; broadcast reference maximum is 7%",
  },
  vignette: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.12,
    desc: "Edge light loss through curved faceplate glass",
  },
  damperWireStrength: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.2,
    desc: "Horizontal grille-stabilizing wires on aperture-grille profiles",
    visibleWhen: (options: any) => options.tubeProfile === CRT_PROFILE.APERTURE_GRILLE,
  },
  interlace: {
    type: BOOL,
    default: false,
    desc: "Simulate interlaced scanning in Custom mode",
    visibleWhen: (options: any) => options.tubeProfile === CRT_PROFILE.CUSTOM,
  },
  persistence: {
    type: RANGE,
    label: "Phosphor Persistence Afterglow",
    range: [0, 1],
    step: 0.01,
    default: 0,
    desc: "Phosphor afterglow persistence",
  },
  flicker: {
    type: RANGE,
    range: [0, 0.15],
  ...
  animSpeed: {
    ...
    step: 1,
    default: 30,
    desc: "Preview frame rate for flicker, interlace, persistence, and degauss",
  },
  animate: {
    type: ACTION,
    label: "Play / Stop",
  ...
  blur: {
    type: BOOL,
    default: false,
    desc: "Apply a legacy final Gaussian blur after the physical display model",
  },
  palette: {
    type: PALETTE,
    default: palettes.nearest,
    desc: "Optional output palette quantization after display encoding",
  },
};
...
const rgbStripe = (input: any, options: RgbStripeOptions = defaults) => {
  ...
  const resolvedBeamMinWidth = resolveCrtProfileSetting(
    beamMinWidth,
    defaults.beamMinWidth,
    profile.beamMinSigma,
  );
  const resolvedBeamMaxWidth = resolveCrtProfileSetting(
    beamMaxWidth,
    defaults.beamMaxWidth,
    profile.beamMaxSigma,
  );
  const resolvedCornerFocus = resolveCrtProfileSetting(
    cornerFocus,
    defaults.cornerFocus,
    profile.cornerFocus,
  );
  const resolvedCurvature = resolveCrtProfileSetting(
    curvature,
    defaults.curvature,
    profile.curvature,
  );
  ...
  logFilterBackend(
    "rgbStripe",
    "WebGL2",
    `${tubeProfile} mask=${resolvedMask}${quantizeInShader ? "" : "+palettePass"}${blur ? "+blur" : ""}`,
  );
  return output;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Profiled CRT display model with voltage-to-light transfer, beam-current scanlines, phosphor masks, overscan, convergence, and faceplate bloom",
  requiresGL: true,
  temporal: true,
});
