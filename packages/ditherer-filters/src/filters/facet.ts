...
export const optionTypes = {
  facetSize: {
    ...
    step: 1,
    default: 18,
    desc: "Average width of each faceted cell",
  },
  jitter: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.35,
    desc: "Randomize each cell center for a less rigid grid",
  },
  seed: {
    type: RANGE,
    range: [0, 9999],
    step: 1,
    default: 1618,
    desc: "Deterministic layout variation independent of size and jitter",
  },
  seamWidth: {
    type: RANGE,
    range: [0, 6],
    step: 1,
    default: 1,
    desc: "Dark seam width between facets",
  },
...
const facet = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  logFilterBackend(
    "Facet",
    "WebGL2",
    `size=${facetSize} fill=${fillMode}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Break the image into seeded faceted planes with antialiased seams and center or local-mean color",
  requiresGL: true,
});
