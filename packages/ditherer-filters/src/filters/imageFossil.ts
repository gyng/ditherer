...
export const optionTypes = {
  depth: {
    ...
    step: 0.1,
    default: 4.5,
    desc: "Depth of the embedded luminance relief",
  },
  strata: {
    type: RANGE,
    range: [2, 40],
    step: 1,
    default: 14,
    desc: "Number and tightness of sediment layers",
  },
  cracks: {
    type: RANGE,
    range: [0.1, 3],
    step: 0.1,
    default: 1.2,
    desc: "Density of cellular stone fractures",
  },
  mineral: {
    type: RANGE,
    range: [0, 3],
    step: 0.05,
    default: 0.55,
    desc: "Blue mineral emission inside cracks",
  },
  lightAngle: {
    type: RANGE,
    range: [0, 360],
    step: 1,
    default: 135,
    desc: "Directional light across the fossil relief",
  },
  stoneColor: { type: COLOR, default: [172, 151, 117], desc: "Sedimentary stone color" },
  fossilColor: {
    type: COLOR,
    default: [92, 70, 48],
    desc: "Color of the embedded source impression",
  },
  palette: { type: PALETTE, default: nearest, desc: "Optional output palette quantization" },
};
export const defaults = {
  depth: optionTypes.depth.default,
  strata: optionTypes.strata.default,
  cracks: optionTypes.cracks.default,
  mineral: optionTypes.mineral.default,
  lightAngle: optionTypes.lightAngle.default,
  stoneColor: optionTypes.stoneColor.default,
  fossilColor: optionTypes.fossilColor.default,
  palette: { ...optionTypes.palette.default, options: { levels: 256 } },
};
const imageFossil = (input: HTMLCanvasElement | OffscreenCanvas, options = defaults) => {
  const W = input.width,
    H = input.height;
  const rendered = renderGLSinglePass({
    ...
    setUniforms: (gl, u) => {
      ...
      gl.uniform1f(u.u_strata, options.strata);
      gl.uniform1f(u.u_cracks, options.cracks);
      gl.uniform1f(u.u_mineral, options.mineral);
      gl.uniform1f(u.u_lightAngle, options.lightAngle);
      gl.uniform3f(
        u.u_stoneColor,
        options.stoneColor[0],
        options.stoneColor[1],
        options.stoneColor[2],
      );
      gl.uniform3f(
        u.u_fossilColor,
        options.fossilColor[0],
        options.fossilColor[1],
        options.fossilColor[2],
      );
    },
  });
  ...
  logFilterBackend(
    "Image Fossil",
    "WebGL2",
    `strata=${options.strata}${identity ? "" : "+palettePass"}`,
  );
  return identity
    ? rendered
