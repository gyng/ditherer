...
export const optionTypes = {
  levelsR: {
    ...
    step: 1,
    default: 4,
    desc: "Quantization levels for red channel",
  },
  levelsG: {
    type: RANGE,
    range: [2, 32],
    step: 1,
    default: 8,
    desc: "Quantization levels for green channel",
  },
  levelsB: {
    type: RANGE,
    range: [2, 32],
    step: 1,
    default: 3,
    desc: "Quantization levels for blue channel",
  },
...
const chromaticPosterize = (input: any, options: typeof defaults = defaults) => {
  ...
  logFilterBackend(
    "Chromatic Posterize",
    "WebGL2",
    `R=${levelsR} G=${levelsG} B=${levelsB}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
