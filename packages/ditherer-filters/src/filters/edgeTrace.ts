...
export const optionTypes = {
  threshold: {
    ...
    step: 1,
    default: 30,
    desc: "Minimum gradient strength — higher values trace fewer, stronger edges",
  },
  ...
  overlayMix: {
    ...
    step: 0.05,
    default: 0.7,
    desc: "How strongly traced lines blend over the source image",
    visibleWhen: (options: any) => options.renderMode === RENDER_MODE.OVERLAY,
  },
  bgColor: {
    type: COLOR,
    default: [255, 255, 255],
    desc: "Solid-mode background color",
    visibleWhen: (options: any) => options.renderMode !== RENDER_MODE.OVERLAY,
  },
...
const edgeTrace = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const renderMode =
    options.renderMode === RENDER_MODE.OVERLAY ? RENDER_MODE.OVERLAY : RENDER_MODE.SOLID;
  ...
  const rendered = renderEdgeTraceGL(
    ...
    renderMode === RENDER_MODE.OVERLAY,
    overlayMix,
  );
  ...
  logFilterBackend(
    "Edge Trace",
    "WebGL2",
    `threshold=${threshold} mode=${renderMode}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
