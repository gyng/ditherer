...
export const optionTypes = {
  amountX: {
    ...
    step: 1,
    default: 6,
    desc: "Maximum horizontal wobble in pixels",
  },
  amountY: {
    type: RANGE,
    range: [0, 40],
    step: 1,
    default: 4,
    desc: "Maximum vertical wobble in pixels",
  },
  rotation: {
    type: RANGE,
    range: [0, 12],
    step: 0.1,
    default: 1.2,
    desc: "Maximum rotational wobble in degrees",
  },
  zoomJitter: {
    type: RANGE,
    range: [0, 0.25],
    step: 0.01,
    default: 0.04,
    desc: "Subtle zoom breathing mixed into the wobble",
  },
  frequency: {
    type: RANGE,
    range: [0.1, 4],
    step: 0.1,
    default: 1.2,
    desc: "How quickly the periodic wobble evolves over time",
  },
  animSpeed: {
    type: RANGE,
    range: [1, 30],
...
const rhythmicWobble = (input: any, options: RhythmicWobbleOptions = defaults) => {
  ...
  const angle =
    (Math.sin(phaseR) * rotation + Math.cos(phaseR * 1.87) * rotation * 0.45) * (Math.PI / 180);
  ...
  logFilterBackend(
    "Rhythmic Wobble",
    "WebGL2",
    `offsetX=${offsetX.toFixed(2)} offsetY=${offsetY.toFixed(2)}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
