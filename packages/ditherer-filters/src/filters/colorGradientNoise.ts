...
export const optionTypes = {
  scale: {
    ...
    step: 1,
    default: 50,
    desc: "Noise feature size in pixels",
  },
  ...
  mix: {
    ...
    step: 0.01,
    default: 0.4,
    desc: "Blend amount with source image",
  },
  seed: {
    type: RANGE,
    range: [0, 999],
...
const colorGradientNoise = (input: any, options: typeof defaults = defaults) => {
  const { scale, color1, color2, mix, seed, palette } = options;
  ...
  logFilterBackend(
    ...
    "WebGL2",
    `scale=${scale} mix=${mix}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
