...
export const optionTypes = {
  autoIris: { type: BOOL, default: true, desc: "Meter the center 40% of the frame, matching the camera's automatic iris region" },
  exposure: { type: RANGE, range: [-2, 2], step: 0.05, default: 0, desc: "Manual exposure compensation in stops, applied around the automatic iris" },
  contrast: { type: RANGE, range: [0.5, 2.5], step: 0.05, default: 1.25, desc: "Monochrome video contrast after CCD exposure and AGC" },
  signalBandwidth: { type: RANGE, range: [0.25, 1], step: 0.05, default: 0.5, desc: "Luma bandwidth relative to the 180 kHz pixel stream; 0.5 matches the documented 90 kHz low-pass filter" },
  cassetteNoise: { type: RANGE, range: [0, 0.3], step: 0.01, default: 0.055, desc: "Deterministic FM cassette grain added at the recorded signal stage" },
  dropout: { type: RANGE, range: [0, 0.15], step: 0.005, default: 0.012, desc: "Probability of short horizontal tape signal losses per CCD capture" },
  tracking: { type: RANGE, range: [0, 6], step: 0.1, default: 0.7, desc: "Horizontal line displacement from cassette transport instability, in sensor pixels" },
  animSpeed: { type: RANGE, range: [15, 60], step: 1, default: 30, desc: "Preview rate; the 15 Hz CCD cadence is retained exactly at any selected rate" },
  animate: {
    type: ACTION,
    label: "Play / Stop",
...
type PxlOptions = FilterOptionValues & Partial<typeof defaults> & {
  _frameIndex?: number;
...
const pxl2000 = (input: FilterCanvas, options: PxlOptions = defaults): FilterCanvas => {
  ...
  program ??= linkProgram(gl, FS, [
    "u_source", "u_history", "u_res", "u_haveHistory", "u_newCapture", "u_autoIris", "u_exposure",
    "u_contrast", "u_bandwidth", "u_noise", "u_dropout", "u_tracking", "u_capture",
  ]);
  ...
  if (haveHistory) {
    ...
    gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, input.width, input.height, gl.RGBA, gl.UNSIGNED_BYTE, previous!);
  }
  ...
  drawPass(gl, null, input.width, input.height, program, () => {
    ...
    gl.uniform1f(program?.uniforms.u_exposure ?? null, clamp(options.exposure, defaults.exposure, -2, 2));
    gl.uniform1f(program?.uniforms.u_contrast ?? null, clamp(options.contrast, defaults.contrast, 0.5, 2.5));
    gl.uniform1f(program?.uniforms.u_bandwidth ?? null, clamp(options.signalBandwidth, defaults.signalBandwidth, 0.25, 1));
    gl.uniform1f(program?.uniforms.u_noise ?? null, clamp(options.cassetteNoise, defaults.cassetteNoise, 0, 0.3));
    gl.uniform1f(program?.uniforms.u_dropout ?? null, clamp(options.dropout, defaults.dropout, 0, 0.15));
    gl.uniform1f(program?.uniforms.u_tracking ?? null, clamp(options.tracking, defaults.tracking, 0, 6));
  ...
  logFilterBackend("PXL-2000", "WebGL2", `120x90 CCD capture ${timing.captureIndex} at 15 Hz + FM cassette`);
  return output;
};
...
export default defineFilter({
  ...
  defaults,
  options: defaults,
  description: "Fisher-Price PXL-2000: 120×90 monochrome CCD, 15 Hz ping-pong capture, 90 kHz luma, and FM cassette defects",
  requiresGL: true,
  temporal: true,
  autoAnimate: true,
  autoAnimateFps: 30,
