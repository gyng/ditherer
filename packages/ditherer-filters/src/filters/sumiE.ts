...
export const optionTypes = {
  brushSoftness: {
    ...
    step: 0.5,
    default: 3,
    desc: "Gaussian sigma for the wash base — higher = looser, more continuous washes",
  },
  washLevels: {
    type: RANGE,
    range: [2, 6],
    step: 1,
    default: 4,
    desc: "Number of discrete ink-dilution bands (deep black, dark, mid, light)",
  },
  washStrength: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.75,
    desc: "Overall ink density — 0 leaves the paper, 1 saturates darks fully",
  },
  washSoftness: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.35,
    desc: "Smoothness of the band boundaries — 0 hard poster, 1 continuous",
  },
  edgeThreshold: {
    type: RANGE,
    range: [0, 0.8],
    step: 0.02,
    default: 0.18,
    desc: "Sobel gradient cutoff — raise to show fewer, more confident strokes",
  },
  edgeStrength: {
    type: RANGE,
    range: [0, 1.5],
    step: 0.05,
    default: 0.5,
    desc: "Ink intensity of the brush strokes at strong edges",
  },
  inkColor: {
    type: COLOR,
    default: [28, 24, 22],
    desc: "Ink colour for the darkest washes and brush strokes",
  },
  ...
  grain: {
    ...
    step: 0.02,
    default: 0.12,
    desc: "Correlated long-fibre washi formation and ink resistance",
  },
  palette: {
    type: PALETTE,
    default: nearest,
    desc: "Optional palette applied after the ink-and-paper composite",
  },
};
...
const sumiE = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  logFilterBackend(
    "Sumi-e",
    "WebGL2",
    `levels=${washLevels} brush=${brushSoftness}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
