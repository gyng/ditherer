...
export const optionTypes = {
  height: {
    ...
    step: 0.1,
    default: 4,
    desc: "Luminance height scale used by the horizon cones",
  },
  radius: {
    type: RANGE,
    range: [2, 120],
    step: 2,
    default: 42,
    desc: "Ambient-occlusion search radius in pixels",
  },
  strength: {
    type: RANGE,
    range: [0, 12],
    step: 0.25,
    default: 5,
    desc: "Crease and contact-shadow strength",
  },
  bias: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.08,
    desc: "Height bias that prevents flat surfaces self-occluding",
  },
  mode: {
    type: ENUM,
    options: [
      { name: "Original color", value: MODE.ORIGINAL },
      { name: "Stone material", value: MODE.STONE },
      { name: "AO only", value: MODE.AO_ONLY },
    ],
    default: MODE.ORIGINAL,
    desc: "Base material beneath the occlusion",
  },
...
const coneTracedAO = (input: HTMLCanvasElement | OffscreenCanvas, options = defaults) => {
  const W = input.width,
    H = input.height;
  const rendered = renderGLSinglePass({
    ...
    key: "coneTracedAO",
    fragmentShader: FS,
    uniformNames: ["u_height", "u_radius", "u_strength", "u_bias", "u_mode", "u_stoneColor"],
    setUniforms: (gl, u) => {
      gl.uniform1f(u.u_height, options.height);
      gl.uniform1f(u.u_radius, options.radius);
      gl.uniform1f(u.u_strength, options.strength);
      gl.uniform1f(u.u_bias, options.bias);
      gl.uniform1i(
        u.u_mode,
        options.mode === MODE.STONE ? 1 : options.mode === MODE.AO_ONLY ? 2 : 0,
      );
      gl.uniform3f(
        u.u_stoneColor,
