...
export const optionTypes = {
  strength: {
    ...
    step: 1,
    default: 20,
    desc: "Maximum pixel displacement",
  },
  direction: {
    type: ENUM,
    options: [
  ...
  blurRadius: {
    ...
    step: 1,
    default: 15,
    desc: "Blur radius when using blurred warp source",
  },
...
const displace = (input: any, options = defaults) => {
  ...
  if (
    ...
    const levels = isNearest
      ...
      : 256;
    const rendered = renderDisplaceGL(
      input,
      W,
      H,
      strength,
  ...
  const warpMap = warpSource === WARP_BLURRED ? blurLuminance(rawLum, W, H, blurRadius) : rawLum;
  ...
  for (let x = 0; x < W; x += 1) {
    for (let y = 0; y < H; y += 1) {
      ...
      const srcX =
        direction !== DIRECTION_Y ? Math.max(0, Math.min(W - 1, Math.round(x + disp))) : x;
      const srcY =
        direction !== DIRECTION_X ? Math.max(0, Math.min(H - 1, Math.round(y + disp))) : y;
