...
export const optionTypes = {
  k1: {
    ...
    step: 0.01,
    default: 0.3,
    desc: "Primary distortion (+barrel, -pincushion)",
  },
  k2: {
    type: RANGE,
    range: [-1, 1],
    step: 0.01,
    default: 0,
    desc: "Secondary radial distortion for fine-tuning edges",
  },
  zoom: {
    type: RANGE,
    range: [0.1, 3],
    step: 0.01,
    default: 1,
    desc: "Zoom factor to compensate for distortion cropping",
  },
...
const lensDistortion = (input: any, options: typeof defaults = defaults) => {
  ...
  logFilterBackend(
    "Lens distortion",
    "WebGL2",
    `k1=${k1} k2=${k2} zoom=${zoom}${isNearest ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
