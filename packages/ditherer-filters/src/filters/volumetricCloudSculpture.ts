...
export const optionTypes = {
  density: {
    ...
    step: 0.1,
    default: 2.8,
    desc: "Optical density of the source-shaped cloud",
  },
  depth: {
    type: RANGE,
    range: [0.5, 2],
    step: 0.05,
    default: 1.1,
    desc: "Depth of the sculpted volume",
  },
  detail: {
    type: RANGE,
    range: [0.5, 12],
    step: 0.25,
    default: 4,
    desc: "Scale of internal turbulent cloud detail",
  },
  scatter: {
    type: RANGE,
    range: [0, 3],
    step: 0.05,
    default: 1.15,
    desc: "Light scattering through low-density regions",
  },
  lightAngle: {
    type: RANGE,
    range: [0, 360],
    step: 1,
    default: 45,
    desc: "Direction of the volumetric light",
  },
  steps: {
    type: RANGE,
    range: [24, 96],
    step: 8,
    default: 64,
    desc: "Volume-integration sample count",
  },
  speed: {
    type: RANGE,
    range: [0, 2],
    step: 0.05,
    default: 0.35,
    desc: "Camera orbit and cloud-evolution speed",
  },
  background: { type: COLOR, default: [9, 12, 28], desc: "Color behind the cloud sculpture" },
...
const volumetricCloudSculpture = (
  ...
  const rendered = renderGLSinglePass({
    ...
    setUniforms: (gl, u) => {
      ...
      gl.uniform1f(u.u_scatter, options.scatter);
      gl.uniform1f(u.u_lightAngle, options.lightAngle);
      gl.uniform1f(u.u_steps, options.steps);
      gl.uniform1f(u.u_time, (runtime._frameIndex ?? 0) * options.speed * 0.025);
      gl.uniform3f(
        u.u_background,
...
export default defineFilter({
  name: "Volumetric Cloud Sculpture",
  func: volumetricCloudSculpture,
  optionTypes,
  options: defaults,
  defaults,
  description:
