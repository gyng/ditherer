...
const buildBayer = (order: number): number[][] => {
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      const py = y % n;
      const px = x % n;
      const quadrant = (y < n ? 0 : 2) + (x < n ? 0 : 1);
...
export const optionTypes = {
  levelsR: {
    ...
    step: 1,
    default: 4,
    desc: "Quantization levels for red",
  },
  levelsG: {
    type: RANGE,
    range: [2, 16],
    step: 1,
    default: 4,
    desc: "Quantization levels for green",
  },
  levelsB: {
    type: RANGE,
    range: [2, 16],
    step: 1,
    default: 4,
    desc: "Quantization levels for blue",
  },
  matrixSize: {
    type: ENUM,
    options: [
...
const posterizeDither = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  logFilterBackend(
    "Posterize Dither",
    "WebGL2",
    `matrix=${matrixSize}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
