...
export const optionTypes = {
  depth: {
    ...
    step: 1,
    default: 8,
    desc: "How far the pixel slabs extrude in isometric space",
  },
  direction: {
    type: ENUM,
    options: [
  ...
  threshold: {
    ...
    step: 1,
    default: 12,
    desc: "Skip near-black or near-transparent pixels below this luma threshold",
  },
  shadowColor: {
    type: COLOR,
    default: [32, 24, 20],
    desc: "Color of the extrusion side wall / shadow",
  },
  shadeFalloff: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.65,
    desc: "How quickly the side wall fades with depth",
  },
...
const isometricExtrude = (input: any, options: typeof defaults = defaults) => {
  ...
  logFilterBackend(
    "Isometric Extrude",
    "WebGL2",
    `depth=${depth} dir=${direction}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
