...
export const optionTypes = {
  ...
  blurRadius: {
    ...
    step: 1,
    default: 5,
    desc: "Pre-blur the displacement map for smoother warps",
  },
  channelX: {
    type: ENUM,
    options: [
    ...
    default: CHANNEL.R,
    desc: "Color channel driving horizontal displacement",
  },
  channelY: {
    type: ENUM,
    options: [
    ...
    default: CHANNEL.G,
    desc: "Color channel driving vertical displacement",
  },
...
const displacementMapXY = (input: any, options: typeof defaults = defaults) => {
  ...
  logFilterBackend(
    "Displacement Map XY",
    "WebGL2",
    `strength=${strength} blur=${blurRadius}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};

export default defineFilter({
  name: "Displacement Map XY",
  func: displacementMapXY,
  optionTypes,
  options: defaults,
  defaults,
  requiresGL: true,
