...
export const optionTypes = {
  strength: {
    ...
    step: 0.05,
    default: 0.5,
    desc: "Spherize (+) or flatten (-) intensity",
  },
  centerX: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.5,
    desc: "Horizontal center of distortion",
  },
  centerY: {
    type: RANGE,
    range: [0, 1],
    step: 0.01,
    default: 0.5,
    desc: "Vertical center of distortion",
  },
  radius: {
    type: RANGE,
    range: [0.1, 1.5],
    step: 0.05,
    default: 0.8,
    desc: "Affected area radius as fraction of image",
  },
    ...
    if (rendered) {
      ...
      if (out) {
        logFilterBackend(
          "Spherize",
          "WebGL2",
          `strength=${strength}${isNearest ? ` levels=${levels}` : "+palettePass"}`,
        );
        return out;
      }
    }
