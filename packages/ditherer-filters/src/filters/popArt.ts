...
export const optionTypes = {
  dotSize: {
    ...
    default: 6,
    label: "Screen pitch",
    desc: "Spacing of the Ben-Day dot screen in pixels",
  },
  ...
  saturationBoost: {
    ...
    step: 0.1,
    default: 2,
    desc: "Vivid color saturation multiplier",
  },
  screenAngle: {
    type: RANGE,
    range: [0, 90],
    step: 1,
    default: 15,
    desc: "Rotation angle of the Ben-Day dot screen",
  },
  paperColor: {
    type: COLOR,
    default: [255, 248, 235],
    desc: "Paper color visible between printed dots",
  },
...
const popArt = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const rendered = renderPopArtGL(input, W, H, dotSize, levels, saturationBoost, screenAngle, [
    paperColor[0],
    paperColor[1],
    paperColor[2],
  ]);
  ...
  logFilterBackend(
    "Pop Art",
    "WebGL2",
    `dotSize=${dotSize} levels=${levels}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Posterize color into an area-correct, rotatable Ben-Day screen on configurable paper",
  requiresGL: true,
});
