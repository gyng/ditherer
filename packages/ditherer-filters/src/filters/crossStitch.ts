...
import { rgba, srgbPaletteGetColor, logFilterBackend } from "../utils/index";
...
export const optionTypes = {
  stitchSize: {
    ...
    step: 1,
    default: 12,
    desc: "Size of each stitched tile in pixels",
  },
  threadColor: {
    type: ENUM,
    options: [
  ...
  fabricColor: {
    type: COLOR,
    default: [240, 232, 214],
    desc: "Fabric base color behind the stitches",
  },
  gapBetween: {
    type: RANGE,
    range: [0, 4],
    step: 1,
    default: 1,
    desc: "Padding between neighboring stitched tiles",
  },
...
const crossStitch = (input: any, options: typeof defaults = defaults) => {
  ...
  const fabricForShader = needsPalettePass
    ? srgbPaletteGetColor(
        palette,
        rgba(fabricColor[0], fabricColor[1], fabricColor[2], 255),
        palette.options,
      )
  ...
  logFilterBackend(
    "Cross Stitch",
    "WebGL2",
    `size=${stitchSize} mode=${threadColor}${needsPalettePass ? "+palettePass" : ""}`,
  );
  return out ?? input;
};
