...
export const optionTypes = {
  fractal: {
    type: ENUM,
    options: [
      { name: "Mandelbulb", value: FRACTAL.MANDELBULB },
      { name: "Julia bulb", value: FRACTAL.JULIA_BULB },
    ],
  ...
  power: {
    ...
    step: 0.25,
    default: 8,
    desc: "Bulb exponent controlling lobes and symmetry",
  },
  iterations: {
    type: RANGE,
    range: [3, 12],
    step: 1,
    default: 8,
    desc: "Distance-estimator iterations",
  },
  detail: {
    type: RANGE,
    range: [0.0005, 0.01],
    step: 0.0005,
    default: 0.002,
    desc: "Surface hit tolerance; lower reveals finer detail",
  },
  yaw: {
    type: RANGE,
    range: [-180, 180],
    step: 1,
    default: 25,
    desc: "Camera orbit around the fractal",
  },
  ...
  zoom: {
    ...
    step: 0.05,
    default: 3.1,
    desc: "Camera distance from the fractal",
  },
  materialMix: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.75,
    desc: "Use source image color instead of procedural orbit color",
  },
  rotateSpeed: {
    type: RANGE,
    range: [0, 2],
    step: 0.05,
    default: 0.18,
    desc: "Automatic portal rotation speed",
  },
...
const fractalPortal = (input: HTMLCanvasElement | OffscreenCanvas, options = defaults) => {
  ...
  logFilterBackend(
    "Fractal Portal",
    "WebGL2",
    `${options.fractal} iter=${options.iterations}${identity ? "" : "+palettePass"}`,
  );
  return identity
    ? rendered
