...
export const optionTypes = {
  density: {
    ...
    step: 0.01,
    default: 0.98,
    desc: "Ink-holding density of the fully rocked plate ground",
  },
  dotSize: {
    type: RANGE,
    range: [1, 8],
    step: 1,
    default: 2,
    desc: "Scale of the rocker-tooth grain in output pixels",
  },
  burnish: {
    type: RANGE,
    range: [0.4, 2],
    step: 0.05,
    default: 1.35,
    desc: "Tonal curve created by scraping and burnishing the dark ground toward paper",
  },
  burrStrength: {
    type: RANGE,
    range: [0, 0.5],
    step: 0.01,
    default: 0.13,
    desc: "Visibility of the crossed ink-holding rocker burr",
  },
  plateWear: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.03,
    desc: "Flatten the fragile burr as though later impressions were pulled from a worn plate",
  },
...
const mezzotint = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const { density, dotSize, burnish, burrStrength, plateWear, inkColor, paperColor, palette } =
    resolved;
  const W = input.width,
    H = input.height;
  ...
  const rendered = renderMezzotintGL(
    ...
    burrStrength,
    plateWear,
    inkColor as [number, number, number],
    paperColor as [number, number, number],
  );
  ...
  logFilterBackend(
    "Mezzotint",
    "WebGL2",
    `density=${density} dotSize=${dotSize}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Dark-ground tonal mezzotint with crossed rocker burr, scraped and burnished light, plate wear, ink, and paper",
  requiresGL: true,
});
