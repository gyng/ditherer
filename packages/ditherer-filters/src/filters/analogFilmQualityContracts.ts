...
const densityHash = (x: number, y: number, seed: number): number => {
  let value =
    Math.imul(Math.floor(x), 0x1f123bb5) ^ Math.imul(Math.floor(y), 0x5f356495) ^ (seed | 0);
...
export const projectionArtifactCounts = (
  ...
  const dust = Math.min(
    64,
    Math.max(0, Math.round(unit(dustAmount) * 24 * areaScale * varied(dustJitter))),
  );
  const scratches =
    unit(scratchAmount) === 0
      ? 0
      : Math.min(
          16,
          Math.max(1, Math.round(unit(scratchAmount) * 6 * widthScale * varied(scratchJitter))),
        );
...
const linearToSrgb = (value: number): number => {
  ...
  return unit(channel <= 0.0031308 ? channel * 12.92 : 1.055 * channel ** (1 / 2.4) - 0.055);
};
...
export const linearLightLeakChannel = (
  ...
  leakSrgb: number,
  exposure: number,
): number => linearToSrgb(srgbToLinear(sourceSrgb) + srgbToLinear(leakSrgb) * unit(exposure));
