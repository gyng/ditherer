...
import { cloneCanvas, fillBufferPixel, getBufferIndex, laba2rgba, logFilterBackend, rgba2labaMemo } from "../utils/index";
...
const bucketRepresentative = (bucket: AdaptiveBucket, adaptMode: string, colorMode: string): number[] => {
...
export const buildMedianCutPalette = (
  ...
  const maxColors = Math.max(1, Math.floor(Number.isFinite(requestedColors) ? requestedColors : defaults.levels));
  const adaptMode = requestedAdaptMode === "MID" || requestedAdaptMode === "FIRST" || requestedAdaptMode === "AVERAGE"
    ? requestedAdaptMode
    : defaults.adaptMode;
  ...
  for (let offset = 0; offset + 3 < buf.length; offset += 4) {
    ...
    if (alpha <= 0) continue;
    const rgb: [number, number, number, number] = [buf[offset] ?? 0, buf[offset + 1] ?? 0, buf[offset + 2] ?? 0, 255];
...
const medianCutFilter = (input: any, options = defaults) => {
  ...
  const levels = Math.max(2, Math.min(32, Math.round(Number(normalized.levels) || defaults.levels)));
  const sampleRate = Math.max(1, Math.min(16, Math.round(Number(normalized.sampleRate) || defaults.sampleRate)));
  ...
  const palette = buildMedianCutPalette(
    new Uint8ClampedArray(sampled.length > 0 ? sampled : buf), levels, adaptMode, colorMode,
  );
