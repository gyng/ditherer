...
type AbaGhostOptions = FilterOptionValues &
  typeof defaults & {
    _frameIndex?: number;
  };
...
const abaGhost = (input: any, options: AbaGhostOptions = defaults) => {
  ...
  const persistence = Math.max(
    0,
    Math.min(0.999, Number(options.persistence ?? defaults.persistence)),
  );
...
export default defineFilter<AbaGhostOptions>({
  ...
  defaults,
  description:
    "Store A and B, then replay a persistent mostly-B double exposure whose emphasized beat can drift off the strict ABA grid for a looser variable-frame ghost trail",
  temporal: true,
  requiresGL: true,
});
