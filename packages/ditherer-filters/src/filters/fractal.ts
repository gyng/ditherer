...
export const optionTypes = {
  ...
  zoom: {
    ...
    step: 0.1,
    default: 1,
    desc: "Zoom level into the fractal",
  },
  centerX: {
    type: RANGE,
    range: [-2.5, 2.5],
    step: 0.01,
    default: -0.5,
    desc: "Horizontal center in complex plane",
  },
  centerY: {
    type: RANGE,
    range: [-2, 2],
    step: 0.01,
    default: 0,
    desc: "Vertical center in complex plane",
  },
  iterations: {
    type: RANGE,
    range: [10, 500],
    step: 10,
    default: 100,
    desc: "Max escape iterations — more = finer detail",
  },
  juliaR: {
    type: RANGE,
    range: [-2, 2],
    step: 0.01,
    default: -0.7,
    desc: "Julia set real component (c.r)",
  },
  juliaI: {
    type: RANGE,
    range: [-2, 2],
    step: 0.01,
    default: 0.27,
    desc: "Julia set imaginary component (c.i)",
  },
  colorSource: {
    type: ENUM,
    options: [
...
const fractalFilter = (input: any, options: typeof defaults = defaults) => {
  const { type, zoom, centerX, centerY, iterations, juliaR, juliaI, colorSource, palette } =
    options;
  const W = input.width;
  ...
  logFilterBackend(
    "Fractal",
    "WebGL2",
    `type=${type} iter=${iterations}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
