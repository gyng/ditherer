...
export const optionTypes = {
  algorithm: {
    ...
    desc: "Pixel-art upscaling algorithm",
  },
...
const copyPixel = (
  ...
  outBuf[di] = srcBuf[si];
  outBuf[di + 1] = srcBuf[si + 1];
  outBuf[di + 2] = srcBuf[si + 2];
  outBuf[di + 3] = srcBuf[si + 3];
};
...
const initCache = (gl: WebGL2RenderingContext): Cache => {
  ...
  _cache = {
    up: linkProgram(gl, UPSCALE_FS, ["u_source", "u_inRes", "u_outRes", "u_algo"] as const),
  };
  return _cache;
};
...
const scale2x = (input: any, options = defaults) => {
  ...
  const getIdx = (x: number, y: number) =>
    getBufferIndex(Math.max(0, Math.min(W - 1, x)), Math.max(0, Math.min(H - 1, y)), W);
    ...
    for (let x = 0; x < W; x++) {
      ...
      if (algorithm === ALGORITHM.NEAREST) {
        ...
        copyPixel(
          ...
          d0,
          buf,
          colorsEqual(buf, A, B) && !colorsEqual(buf, A, C) && !colorsEqual(buf, B, D) ? A : P,
        );
        copyPixel(
          outBuf,
          d1,
          buf,
          colorsEqual(buf, A, C) && !colorsEqual(buf, A, B) && !colorsEqual(buf, C, D) ? A : P,
        );
        copyPixel(
          outBuf,
          d2,
          buf,
          colorsEqual(buf, B, D) && !colorsEqual(buf, A, B) && !colorsEqual(buf, C, D) ? B : P,
        );
        copyPixel(
          outBuf,
          d3,
          buf,
          colorsEqual(buf, C, D) && !colorsEqual(buf, A, C) && !colorsEqual(buf, B, D) ? C : P,
        );
      } else {
        const TL = getIdx(x - 1, y - 1),
          T = getIdx(x, y - 1),
          TR = getIdx(x + 1, y - 1);
        const L = getIdx(x - 1, y),
          R = getIdx(x + 1, y);
        const BL = getIdx(x - 1, y + 1),
          Bo = getIdx(x, y + 1),
          BR = getIdx(x + 1, y + 1);
