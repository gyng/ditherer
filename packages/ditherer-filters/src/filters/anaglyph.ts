...
void main() {
  ...
  if (u_mode == 0) {
    linearRgb = vec3(
      dot(vec3( 0.456100, -0.040082, -0.015216), L) + dot(vec3(-0.043470, -0.087939, -0.001555), R),
      dot(vec3( 0.500484, -0.037824, -0.020597), L) + dot(vec3(-0.378476,  0.733640, -0.018450), R),
      dot(vec3( 0.176381, -0.015759, -0.005468), L) + dot(vec3(-0.072152, -0.112961,  1.226400), R)
    );
  } else {
    float leftY = lum(L);
    float rightY = lum(R);
