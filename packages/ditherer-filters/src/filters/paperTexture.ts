...
export const optionTypes = {
  ...
  scale: {
    ...
    step: 0.5,
    default: 12,
    desc: "Texture density — higher is finer, capped per resolution to prevent moiré",
  },
  strength: {
    type: RANGE,
    range: [0, 1],
    step: 0.05,
    default: 0.6,
    desc: "Texture opacity — 0 = invisible, 1 = fully applied",
  },
  contrast: {
    type: RANGE,
    range: [0.5, 3],
    step: 0.1,
    default: 1.2,
    desc: "Amplify texture variance — makes fibres/grain more pronounced",
  },
  palette: {
    type: PALETTE,
    default: nearest,
    desc: "Optional palette applied after the material texture",
  },
};
...
const paperTexture = (input: any, options: Partial<typeof defaults> = defaults) => {
  ...
  const effectiveScale =
    substratePatternFrequency(scale, Math.min(W, H), repeatDensity) / repeatDensity;

  const rendered = renderPaperTextureGL(
    input,
    W,
    H,
    typeId,
  ...
  logFilterBackend(
    "Paper Texture",
    "WebGL2",
    `${type}/${blendMode} scale=${effectiveScale.toFixed(2)}${identity ? "" : "+palettePass"}`,
  );
  return out ?? input;
};
...
export default defineFilter({
  ...
  defaults,
  description:
    "Procedural paper, canvas, linen, cardboard, or parchment texture overlay — gives digital images material substrate",
  requiresGL: true,
});
