...
export const optionTypes = {
  threshold: {
    ...
    step: 1,
    default: 15,
    desc: "How far a pixel must deviate from its neighbourhood median to be treated as speckle",
  },
  radius: {
    type: RANGE,
    range: [1, 5],
    step: 1,
    default: 2,
    desc: "Neighborhood radius for the median",
  },
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const color = paletteGetColor(
        palette,
        rgba(Math.round(pr), Math.round(pg), Math.round(pb), buf[i + 3]),
        palette.options,
        false,
      );
