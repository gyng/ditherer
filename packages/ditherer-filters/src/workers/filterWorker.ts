...
import { grayscale } from "../filters/index";
import {
  logFilterDispatched,
...
import { glAvailable, glUnavailableStub } from "../gl/index";
import type {
  SerializedOptionMap,
  WorkerFilterRequest,
  WorkerFilterResult,
...
const getOptionObject = (value: unknown): SerializedOptionMap => (isRecord(value) ? value : {});
...
export const runWorkerFilterRequest = async (
  ...
  try {
    ...
    initCtx.putImageData(new ImageData(new Uint8ClampedArray(imageData), width, height), 0, 0);
